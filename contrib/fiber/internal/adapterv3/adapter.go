...
func (a *FiberV3Adapter) RegisterCustomHandler(method, path string, handler http.HandlerFunc) error {
	...
	wrappedHandler := func(c fiber.Ctx) error {
		...
		// Create a minimal *http.Request from the fiber context
		httpReq, err := http.NewRequest(c.Method(), c.OriginalURL(), nil)
		if err != nil {
...
func (w *fiberV3ResponseWriter) Header() http.Header {
	...
	// Fiber v3 doesn't expose fasthttp request headers directly via Context()
	// Return an empty header map - callers should use Fiber's native header API
	return header
}
...
func printServerInfo(framework, addr string) {
	...
	fmt.Printf("  Auth: %s\n", getEnv("CONFIG_AUTH_PROVIDER", "mock"))
