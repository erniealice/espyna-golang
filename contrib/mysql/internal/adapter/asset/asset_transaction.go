...
func init() {
	registry.RegisterRepositoryFactory("mysql", entityid.AssetTransaction, func(conn any, tableName string) (any, error) {
		db, ok := conn.(*sql.DB)
		if !ok {
			return nil, fmt.Errorf("mysql asset_transaction repository requires *sql.DB, got %T", conn)
		}
		dbOps := mysqlCore.NewWorkspaceAwareOperations(db)
		return NewMySQLAssetTransactionRepository(dbOps, tableName), nil
	})
}
...
// MySQLAssetTransactionRepository implements asset_transaction CRUD operations
// using MySQL 8.0+. Mirrors postgres/internal/adapter/asset/asset_transaction.go
// with dialect translation: backtick identifiers, positional ?, no RETURNING.
type MySQLAssetTransactionRepository struct {
	assettxpb.UnimplementedAssetTransactionDomainServiceServer
	dbOps     interfaces.DatabaseOperation
	tableName string
}

// NewMySQLAssetTransactionRepository creates a new MySQL asset_transaction repository.
func NewMySQLAssetTransactionRepository(dbOps interfaces.DatabaseOperation, tableName string) assettxpb.AssetTransactionDomainServiceServer {
	if tableName == "" {
		tableName = "asset_transaction"
	}
	return &MySQLAssetTransactionRepository{
		dbOps:     dbOps,
		tableName: tableName,
	}
}
...
func (r *MySQLAssetTransactionRepository) CreateAssetTransaction(ctx context.Context, req *assettxpb.CreateAssetTransactionRequest) (*assettxpb.CreateAssetTransactionResponse, error) {
	if req.Data == nil {
...
func (r *MySQLAssetTransactionRepository) ReadAssetTransaction(ctx context.Context, req *assettxpb.ReadAssetTransactionRequest) (*assettxpb.ReadAssetTransactionResponse, error) {
...
// ListAssetTransactions lists asset_transaction rows using common MySQL operations.
func (r *MySQLAssetTransactionRepository) ListAssetTransactions(ctx context.Context, req *assettxpb.ListAssetTransactionsRequest) (*assettxpb.ListAssetTransactionsResponse, error) {
...
func (r *MySQLAssetTransactionRepository) GetAssetTransactionListPageData(
	ctx context.Context,
...
func (r *MySQLAssetTransactionRepository) GetAssetTransactionItemPageData(
	ctx context.Context,
...
// NewAssetTransactionRepository creates a new MySQL asset_transaction repository (old-style constructor).
func NewAssetTransactionRepository(db *sql.DB, tableName string) assettxpb.AssetTransactionDomainServiceServer {
	dbOps := mysqlCore.NewWorkspaceAwareOperations(db)
	return NewMySQLAssetTransactionRepository(dbOps, tableName)
}
