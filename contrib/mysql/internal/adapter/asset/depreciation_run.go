...
func init() {
	registry.RegisterRepositoryFactory("mysql", entityid.DepreciationRun, func(conn any, tableName string) (any, error) {
		db, ok := conn.(*sql.DB)
		if !ok {
			return nil, fmt.Errorf("mysql depreciation_run repository requires *sql.DB, got %T", conn)
		}
		dbOps := mysqlCore.NewWorkspaceAwareOperations(db)
		return NewMySQLDepreciationRunRepository(dbOps, tableName), nil
	})
}
...
// MySQLDepreciationRunRepository implements depreciation_run CRUD + list
// operations using MySQL 8.0+. Mirrors postgres/internal/adapter/asset/depreciation_run.go
// with dialect translation: backtick identifiers, positional ?, no RETURNING.
type MySQLDepreciationRunRepository struct {
	deprunpb.UnimplementedDepreciationRunDomainServiceServer
	dbOps     interfaces.DatabaseOperation
	tableName string
}

// NewMySQLDepreciationRunRepository creates a new MySQL depreciation_run repository.
func NewMySQLDepreciationRunRepository(dbOps interfaces.DatabaseOperation, tableName string) deprunpb.DepreciationRunDomainServiceServer {
	if tableName == "" {
		tableName = "depreciation_run"
	}
	return &MySQLDepreciationRunRepository{
		dbOps:     dbOps,
		tableName: tableName,
	}
}
...
func (r *MySQLDepreciationRunRepository) CreateDepreciationRun(ctx context.Context, req *deprunpb.CreateDepreciationRunRequest) (*deprunpb.CreateDepreciationRunResponse, error) {
	if req.Data == nil {
...
func (r *MySQLDepreciationRunRepository) ReadDepreciationRun(ctx context.Context, req *deprunpb.ReadDepreciationRunRequest) (*deprunpb.ReadDepreciationRunResponse, error) {
...
func (r *MySQLDepreciationRunRepository) UpdateDepreciationRun(ctx context.Context, req *deprunpb.UpdateDepreciationRunRequest) (*deprunpb.UpdateDepreciationRunResponse, error) {
...
func (r *MySQLDepreciationRunRepository) DeleteDepreciationRun(ctx context.Context, req *deprunpb.DeleteDepreciationRunRequest) (*deprunpb.DeleteDepreciationRunResponse, error) {
	if req.Data == nil || req.Data.Id == "" {
		return nil, fmt.Errorf("depreciation_run ID is required")
	}
...
func (r *MySQLDepreciationRunRepository) ListDepreciationRuns(ctx context.Context, req *deprunpb.ListDepreciationRunsRequest) (*deprunpb.ListDepreciationRunsResponse, error) {
...
// NewDepreciationRunRepository creates a new MySQL depreciation_run repository (old-style constructor).
func NewDepreciationRunRepository(db *sql.DB, tableName string) deprunpb.DepreciationRunDomainServiceServer {
	dbOps := mysqlCore.NewWorkspaceAwareOperations(db)
	return NewMySQLDepreciationRunRepository(dbOps, tableName)
}
