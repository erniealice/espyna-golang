...
func init() {
	registry.RegisterRepositoryFactory("mysql", entityid.DepreciationSchedule, func(conn any, tableName string) (any, error) {
		db, ok := conn.(*sql.DB)
		if !ok {
			return nil, fmt.Errorf("mysql depreciation_schedule repository requires *sql.DB, got %T", conn)
		}
		dbOps := mysqlCore.NewWorkspaceAwareOperations(db)
		return NewMySQLDepreciationScheduleRepository(dbOps, tableName), nil
	})
}
...
// MySQLDepreciationScheduleRepository implements depreciation_schedule CRUD
// operations using MySQL 8.0+. Mirrors postgres/internal/adapter/asset/depreciation_schedule.go
// with dialect translation: backtick identifiers, positional ?, no RETURNING.
type MySQLDepreciationScheduleRepository struct {
	depschpb.UnimplementedDepreciationDomainServiceServer
	dbOps     interfaces.DatabaseOperation
	tableName string
}

// NewMySQLDepreciationScheduleRepository creates a new MySQL depreciation_schedule repository.
func NewMySQLDepreciationScheduleRepository(dbOps interfaces.DatabaseOperation, tableName string) depschpb.DepreciationDomainServiceServer {
	if tableName == "" {
		tableName = "depreciation_schedule"
	}
	return &MySQLDepreciationScheduleRepository{
		dbOps:     dbOps,
		tableName: tableName,
	}
}
...
func (r *MySQLDepreciationScheduleRepository) CreateDepreciationSchedule(ctx context.Context, req *depschpb.CreateDepreciationScheduleRequest) (*depschpb.CreateDepreciationScheduleResponse, error) {
	if req.Data == nil {
...
func (r *MySQLDepreciationScheduleRepository) ReadDepreciationSchedule(ctx context.Context, req *depschpb.ReadDepreciationScheduleRequest) (*depschpb.ReadDepreciationScheduleResponse, error) {
...
func (r *MySQLDepreciationScheduleRepository) UpdateDepreciationSchedule(ctx context.Context, req *depschpb.UpdateDepreciationScheduleRequest) (*depschpb.UpdateDepreciationScheduleResponse, error) {
...
func (r *MySQLDepreciationScheduleRepository) DeleteDepreciationSchedule(ctx context.Context, req *depschpb.DeleteDepreciationScheduleRequest) (*depschpb.DeleteDepreciationScheduleResponse, error) {
	if req.Data == nil || req.Data.Id == "" {
		return nil, fmt.Errorf("depreciation_schedule ID is required")
	}
...
func (r *MySQLDepreciationScheduleRepository) ListDepreciationSchedules(ctx context.Context, req *depschpb.ListDepreciationSchedulesRequest) (*depschpb.ListDepreciationSchedulesResponse, error) {
...
func (r *MySQLDepreciationScheduleRepository) GetDepreciationScheduleListPageData(
	ctx context.Context,
...
func (r *MySQLDepreciationScheduleRepository) GetDepreciationScheduleItemPageData(
	ctx context.Context,
...
// NewDepreciationScheduleRepository creates a new MySQL depreciation_schedule repository (old-style constructor).
func NewDepreciationScheduleRepository(db *sql.DB, tableName string) depschpb.DepreciationDomainServiceServer {
	dbOps := mysqlCore.NewWorkspaceAwareOperations(db)
	return NewMySQLDepreciationScheduleRepository(dbOps, tableName)
}
