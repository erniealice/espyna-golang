...
// the hand-written LIMIT/OFFSET list-page adapters. Ported from
// contrib/postgres/internal/adapter/core/order_by_test.go with expectations
// re-quoted to backticks (MySQL identifier quoting). Every emitted ORDER BY must
// terminate in a UNIQUE id key so LIMIT/OFFSET pages over rows that TIE on the
// leading sort key are deterministic, disjoint, and complete.
//
...
// sort column AND the tiebreaker are backtick-quoted PER COMPONENT (`a`.`b`), and
...
func TestBuildOrderByTiebreaker(t *testing.T) {
	...
	// quoted-comma column, to exercise the quote-aware paths.
	...
	cases := []struct {
		...
		// --- W0 dotted-identifier quoting fix (plan §2.4) ---------------------
		{
			...
			// single identifier `pr.date_created` in ONE backtick pair (MySQL reads
			// that as a column literally named "pr.date_created" → Unknown column;
			// payroll_run's whitelist is all-dotted, so its sorts were broken).
		...
		},
		{
			...
			// `x,id` in a single backtick pair, the embedded comma must NOT be split
