...
// Ported from contrib/postgres/internal/adapter/core/order_by_tiebreaker_test.go.
//
// PORT DEVIATION (recorded in docs/plan/20260728-sql-injection-dialect-parity/
// w0-notes.md): the postgres original tests buildListOrderByClause, the GENERIC
// base-List ORDER BY builder (postgres core/operations.go:534). The mysql core
// package has NO buildListOrderByClause — its generic List inlines the ordering
// logic in operations.go:534-553 — so those base-List cases cannot compile here
// and are intentionally not ported. What carries over:
...
//     via the production orderByHasIDKey instead of the postgres test's local
//     bare-token containsIDKey (mysql emits a backtick-quoted `id`).
//
// The simulation models MySQL's freedom to return tied rows in any order per
// fetch and proves the clause BuildOrderBy emits makes LIMIT/OFFSET pages
...
// when the clause does not fully disambiguate tied rows, MySQL may hand them
...
// production quote-aware orderByHasIDKey so the backtick-quoted `id` form is
// recognized.
