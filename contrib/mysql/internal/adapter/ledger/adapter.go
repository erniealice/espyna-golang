...
type TableConfig struct {
	...
	ProductCollection    string // deprecated, kept for backward compat
	Collection           string // deprecated, kept for backward compat
	Line                 string // line table (product taxonomy)
...
// LedgerReportingAdapter implements LedgerReportingService using MySQL 8.0+.
// Dialect differences from the postgres gold standard:
//   - Placeholders: $N → ? (positional, same left-to-right arg order)
//   - Identifiers: "ident" → `ident`
//   - ILIKE → LIKE (utf8mb4_unicode_ci is case-insensitive by default)
//   - FILTER (WHERE c) → CASE WHEN c THEN x END (no FILTER support in MySQL)
//   - LIMIT $N OFFSET $N → LIMIT ? OFFSET ?
//   - COUNT(*) OVER () ✓ (MySQL 8.0+ supports window functions)
//   - All mutations add WHERE workspace_id = ? for multi-tenancy isolation
type LedgerReportingAdapter struct {
	db          *sql.DB
	tableConfig TableConfig
}

// NewLedgerReportingAdapter creates a new MySQL ledger reporting adapter.
...
func (a *LedgerReportingAdapter) GetCashBookReport(
	...
	// Dialect: $1 → ?; $2 → ?; TIMESTAMPTZ cast → DATE_FORMAT; no pg casts
	// DATE_FORMAT(date_created, '%Y-%m-%d') replaces TO_CHAR(date_created, 'YYYY-MM-DD')
	// expenditure_type CASE unchanged — CASE WHEN is standard SQL.
	query := `
		SELECT tx_date, description, reference, tx_type, amount
		FROM (
			SELECT
				DATE_FORMAT(date_created, '%Y-%m-%d') AS tx_date,
			...
			SELECT
				DATE_FORMAT(expenditure_date, '%Y-%m-%d') AS tx_date,
	...
	ws := nilIfEmpty(workspaceID)
	rows, err := a.db.QueryContext(ctx, query, ws, ws, ws, ws, limit)
	if err != nil {
...
//   - ::bigint casts dropped (MySQL infers INT/BIGINT)
func (a *LedgerReportingAdapter) GetSimplePayablesAgingReport(
	...
	query := `
		WITH outstanding AS (
			SELECT
				...
				DATEDIFF(CURDATE(), COALESCE(e.due_date, e.expenditure_date)) AS days_overdue
		...
		SELECT
			supplier_name,
			(COALESCE(SUM(CASE WHEN days_overdue <= 0 THEN outstanding_amount END), 0) * 100) AS current_amt,
			(COALESCE(SUM(CASE WHEN days_overdue BETWEEN 1 AND 30 THEN outstanding_amount END), 0) * 100) AS days_30,
			(COALESCE(SUM(CASE WHEN days_overdue BETWEEN 31 AND 60 THEN outstanding_amount END), 0) * 100) AS days_60,
			(COALESCE(SUM(CASE WHEN days_overdue BETWEEN 61 AND 90 THEN outstanding_amount END), 0) * 100) AS days_90,
			(COALESCE(SUM(CASE WHEN days_overdue > 90 THEN outstanding_amount END), 0) * 100) AS over_90,
			(COALESCE(SUM(outstanding_amount), 0) * 100) AS total
		FROM outstanding
	...
	ws := nilIfEmpty(workspaceID)
	rows, err := a.db.QueryContext(ctx, query, ws, ws)
	if err != nil {
...
func (a *LedgerReportingAdapter) ListRevenue(ctx context.Context, start, end *time.Time) ([]map[string]any, error) {
	...
	// Dialect: $N → ?; no pg casts needed — MySQL driver handles time.Time natively.
...
// CONCAT helper for MySQL (replaces postgres || operator in some queries).
func concatCoalesce(a, sep, b string) string {
	return fmt.Sprintf("CONCAT(COALESCE(%s, ''), '%s', COALESCE(%s, ''))", a, sep, b)
}

// ensure concatCoalesce is referenced to avoid unused-import/function errors.
var _ = concatCoalesce
