...
// MySQL dialect: date_trunc → DATE_FORMAT; EXTRACT → YEAR()/QUARTER(); ::text casts removed.
func getExpenditurePivotDimensionConfig(tc TableConfig, dimension string) pivotDimensionConfig {
	switch dimension {
	case "monthly":
		expr := "DATE_FORMAT(e.expenditure_date, '%Y-%m-01')"
	...
	case "quarterly":
		return pivotDimensionConfig{
			selectKey: "CONCAT('Q', QUARTER(e.expenditure_date), ' ', YEAR(e.expenditure_date))",
			selectID:  "CONCAT(YEAR(e.expenditure_date), '-Q', QUARTER(e.expenditure_date))",
			groupBy:   "YEAR(e.expenditure_date), QUARTER(e.expenditure_date)",
		}
	case "yearly":
		return pivotDimensionConfig{
			selectKey: "CAST(YEAR(e.expenditure_date) AS CHAR)",
			selectID:  "CAST(YEAR(e.expenditure_date) AS CHAR)",
	...
	case "location":
		return pivotDimensionConfig{
			...
			// l is always joined in the base query.
		}
	case "location_area":
		return pivotDimensionConfig{
...
// Dialect: $N → ?; ::timestamptz IS NULL → ? IS NULL; date_trunc → DATE_FORMAT; active = true → active = 1.
...
WITH expenditure_pivot AS (
    SELECT
        ...
        %s AS col_key,
        %s AS col_id,
        SUM(eli.line_amount) AS total_expenditure,
