...
func init() {
	registry.RegisterRepositoryFactory("mysql", entityid.LeaveRequest, func(conn any, tableName string) (any, error) {
		db, ok := conn.(*sql.DB)
		if !ok {
			return nil, fmt.Errorf("mysql leave_request repository requires *sql.DB, got %T", conn)
		}
		dbOps := mysqlCore.NewWorkspaceAwareOperations(db)
		return NewMySQLLeaveRequestRepository(dbOps, tableName), nil
	})
}

// MySQLLeaveRequestRepository implements leave request CRUD operations using MySQL 8.0+.
type MySQLLeaveRequestRepository struct {
	leaverequestpb.UnimplementedLeaveRequestDomainServiceServer
	dbOps     interfaces.DatabaseOperation
	tableName string
}

// NewMySQLLeaveRequestRepository creates a new MySQL leave request repository.
func NewMySQLLeaveRequestRepository(dbOps interfaces.DatabaseOperation, tableName string) leaverequestpb.LeaveRequestDomainServiceServer {
	if tableName == "" {
		tableName = "leave_request"
	}
	return &MySQLLeaveRequestRepository{
		dbOps:     dbOps,
		tableName: tableName,
	}
}
...
func (r *MySQLLeaveRequestRepository) CreateLeaveRequest(ctx context.Context, req *leaverequestpb.CreateLeaveRequestRequest) (*leaverequestpb.CreateLeaveRequestResponse, error) {
	if req.Data == nil {
...
func (r *MySQLLeaveRequestRepository) ReadLeaveRequest(ctx context.Context, req *leaverequestpb.ReadLeaveRequestRequest) (*leaverequestpb.ReadLeaveRequestResponse, error) {
...
func (r *MySQLLeaveRequestRepository) UpdateLeaveRequest(ctx context.Context, req *leaverequestpb.UpdateLeaveRequestRequest) (*leaverequestpb.UpdateLeaveRequestResponse, error) {
	if req.Data == nil || req.Data.Id == "" {
		return nil, fmt.Errorf("leave request ID is required")
	}
	jsonData, err := protojson.Marshal(req.Data)
...
func (r *MySQLLeaveRequestRepository) DeleteLeaveRequest(ctx context.Context, req *leaverequestpb.DeleteLeaveRequestRequest) (*leaverequestpb.DeleteLeaveRequestResponse, error) {
	if req.Data == nil || req.Data.Id == "" {
		return nil, fmt.Errorf("leave request ID is required")
	}
...
func (r *MySQLLeaveRequestRepository) ListLeaveRequests(ctx context.Context, req *leaverequestpb.ListLeaveRequestsRequest) (*leaverequestpb.ListLeaveRequestsResponse, error) {
...
func (r *MySQLLeaveRequestRepository) GetLeaveRequestListPageData(
	ctx context.Context,
...
func (r *MySQLLeaveRequestRepository) GetLeaveRequestItemPageData(
	ctx context.Context,
