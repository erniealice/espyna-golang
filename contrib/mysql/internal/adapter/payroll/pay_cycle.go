...
func init() {
	registry.RegisterRepositoryFactory("mysql", entityid.PayCycle, func(conn any, tableName string) (any, error) {
		db, ok := conn.(*sql.DB)
		if !ok {
			return nil, fmt.Errorf("mysql pay_cycle repository requires *sql.DB, got %T", conn)
		}
		dbOps := mysqlCore.NewWorkspaceAwareOperations(db)
		return NewMySQLPayCycleRepository(dbOps, tableName), nil
	})
}

// MySQLPayCycleRepository implements pay cycle CRUD operations using MySQL 8.0+.
type MySQLPayCycleRepository struct {
	paycyclepb.UnimplementedPayCycleDomainServiceServer
	dbOps     interfaces.DatabaseOperation
	tableName string
}

// NewMySQLPayCycleRepository creates a new MySQL pay cycle repository.
func NewMySQLPayCycleRepository(dbOps interfaces.DatabaseOperation, tableName string) paycyclepb.PayCycleDomainServiceServer {
	if tableName == "" {
		tableName = "pay_cycle"
	}
	return &MySQLPayCycleRepository{
		dbOps:     dbOps,
		tableName: tableName,
	}
}
...
func (r *MySQLPayCycleRepository) CreatePayCycle(ctx context.Context, req *paycyclepb.CreatePayCycleRequest) (*paycyclepb.CreatePayCycleResponse, error) {
	if req.Data == nil {
...
func (r *MySQLPayCycleRepository) ReadPayCycle(ctx context.Context, req *paycyclepb.ReadPayCycleRequest) (*paycyclepb.ReadPayCycleResponse, error) {
...
func (r *MySQLPayCycleRepository) UpdatePayCycle(ctx context.Context, req *paycyclepb.UpdatePayCycleRequest) (*paycyclepb.UpdatePayCycleResponse, error) {
	if req.Data == nil || req.Data.Id == "" {
		return nil, fmt.Errorf("pay cycle ID is required")
	}
	jsonData, err := protojson.Marshal(req.Data)
...
func (r *MySQLPayCycleRepository) DeletePayCycle(ctx context.Context, req *paycyclepb.DeletePayCycleRequest) (*paycyclepb.DeletePayCycleResponse, error) {
	if req.Data == nil || req.Data.Id == "" {
		return nil, fmt.Errorf("pay cycle ID is required")
	}
...
func (r *MySQLPayCycleRepository) ListPayCycles(ctx context.Context, req *paycyclepb.ListPayCyclesRequest) (*paycyclepb.ListPayCyclesResponse, error) {
...
func (r *MySQLPayCycleRepository) GetPayCycleListPageData(
	ctx context.Context,
...
func (r *MySQLPayCycleRepository) GetPayCycleItemPageData(
	ctx context.Context,
