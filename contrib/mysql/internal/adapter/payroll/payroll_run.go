...
func init() {
	registry.RegisterRepositoryFactory("mysql", entityid.PayrollRun, func(conn any, tableName string) (any, error) {
		db, ok := conn.(*sql.DB)
		if !ok {
			return nil, fmt.Errorf("mysql payroll run repository requires *sql.DB, got %T", conn)
		}
		dbOps := mysqlCore.NewWorkspaceAwareOperations(db)
		return NewMySQLPayrollRunRepository(db, dbOps, tableName), nil
	})
}

// MySQLPayrollRunRepository implements payroll run CRUD operations using MySQL 8.0+.
type MySQLPayrollRunRepository struct {
...
// NewMySQLPayrollRunRepository creates a new MySQL payroll run repository.
func NewMySQLPayrollRunRepository(db *sql.DB, dbOps interfaces.DatabaseOperation, tableName string) payrollrunpb.PayrollRunDomainServiceServer {
	if tableName == "" {
		tableName = "payroll_run"
	}
	return &MySQLPayrollRunRepository{
		dbOps:     dbOps,
		db:        db,
		tableName: tableName,
	}
...
func (r *MySQLPayrollRunRepository) CreatePayrollRun(ctx context.Context, req *payrollrunpb.CreatePayrollRunRequest) (*payrollrunpb.CreatePayrollRunResponse, error) {
	if req.Data == nil {
...
func (r *MySQLPayrollRunRepository) ReadPayrollRun(ctx context.Context, req *payrollrunpb.ReadPayrollRunRequest) (*payrollrunpb.ReadPayrollRunResponse, error) {
...
func (r *MySQLPayrollRunRepository) UpdatePayrollRun(ctx context.Context, req *payrollrunpb.UpdatePayrollRunRequest) (*payrollrunpb.UpdatePayrollRunResponse, error) {
	if req.Data == nil || req.Data.Id == "" {
		return nil, fmt.Errorf("payroll run ID is required")
	}
	jsonData, err := protojson.Marshal(req.Data)
...
func (r *MySQLPayrollRunRepository) DeletePayrollRun(ctx context.Context, req *payrollrunpb.DeletePayrollRunRequest) (*payrollrunpb.DeletePayrollRunResponse, error) {
	if req.Data == nil || req.Data.Id == "" {
		return nil, fmt.Errorf("payroll run ID is required")
	}
...
func (r *MySQLPayrollRunRepository) ListPayrollRuns(ctx context.Context, req *payrollrunpb.ListPayrollRunsRequest) (*payrollrunpb.ListPayrollRunsResponse, error) {
...
//   - $1::text IS NULL OR $1::text = ” → ? IS NULL OR ? = ” (MySQL, no casts)
//     Simplified to a single workspace_id check since MySQL handles NULLs natively.
//   - ILIKE → LIKE (MySQL ci collation)
//   - $N positional → ? (positional, re-sequenced)
//   - mysqlCore.BuildOrderBy uses backtick quoting
func (r *MySQLPayrollRunRepository) GetPayrollRunListPageData(
	ctx context.Context,
	...
	// A2: Sort guard — fail-closed via BuildOrderBy whitelist; backtick quoting.
	orderByClause, err := mysqlCore.BuildOrderBy(payrollRunSortableSQLCols, req.GetSort(), "pr.date_created DESC")
	if err != nil {
		return nil, err
	}

	// A3: COUNT(*) OVER() — MySQL 8.0+ window functions supported.
	// Dialect: ? placeholders; (? IS NULL OR ? = '' OR pr.workspace_id = ?) replaces $1::text IS NULL.
	// LIKE replaces ILIKE.
	query := fmt.Sprintf(`
		SELECT
			pr.id,
		...
		WHERE (? IS NULL OR ? = '' OR pr.workspace_id = ?)
		  AND (? IS NULL OR ? = '' OR pr.run_number LIKE ?)
		%s
		LIMIT ? OFFSET ?;
...
func (r *MySQLPayrollRunRepository) GetPayrollRunItemPageData(
	ctx context.Context,
...
// NewPayrollRunRepository creates a new MySQL payroll run repository (old-style constructor).
func NewPayrollRunRepository(db *sql.DB, tableName string) payrollrunpb.PayrollRunDomainServiceServer {
	dbOps := mysqlCore.NewWorkspaceAwareOperations(db)
	return NewMySQLPayrollRunRepository(db, dbOps, tableName)
}
