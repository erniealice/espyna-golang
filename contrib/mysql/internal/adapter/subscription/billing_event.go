...
func init() {
	registry.RegisterRepositoryFactory("mysql", entityid.BillingEvent, func(conn any, tableName string) (any, error) {
		db, ok := conn.(*sql.DB)
		if !ok {
			return nil, fmt.Errorf("mysql billing_event repository requires *sql.DB, got %T", conn)
		}
		dbOps := mysqlCore.NewWorkspaceAwareOperations(db)
		return NewMySQLBillingEventRepository(dbOps, tableName), nil
	})
}

// MySQLBillingEventRepository implements billing_event CRUD operations using MySQL 8.0+.
type MySQLBillingEventRepository struct {
...
// NewMySQLBillingEventRepository creates a new MySQL billing_event repository.
func NewMySQLBillingEventRepository(dbOps interfaces.DatabaseOperation, tableName string) pb.BillingEventDomainServiceServer {
	if tableName == "" {
	...
	return &MySQLBillingEventRepository{
		dbOps:     dbOps,
		db:        db,
		tableName: tableName,
	}
...
func (r *MySQLBillingEventRepository) CreateBillingEvent(ctx context.Context, req *pb.CreateBillingEventRequest) (*pb.CreateBillingEventResponse, error) {
	if req.Data == nil {
...
func (r *MySQLBillingEventRepository) ReadBillingEvent(ctx context.Context, req *pb.ReadBillingEventRequest) (*pb.ReadBillingEventResponse, error) {
...
func (r *MySQLBillingEventRepository) UpdateBillingEvent(ctx context.Context, req *pb.UpdateBillingEventRequest) (*pb.UpdateBillingEventResponse, error) {
	if req.Data == nil || req.Data.Id == "" {
		return nil, fmt.Errorf("billing event ID is required")
	}
...
func (r *MySQLBillingEventRepository) DeleteBillingEvent(ctx context.Context, req *pb.DeleteBillingEventRequest) (*pb.DeleteBillingEventResponse, error) {
	if req.Data == nil || req.Data.Id == "" {
		return nil, fmt.Errorf("billing event ID is required")
	}
...
func (r *MySQLBillingEventRepository) ListBillingEvents(ctx context.Context, req *pb.ListBillingEventsRequest) (*pb.ListBillingEventsResponse, error) {
...
//   - CROSS JOIN → COUNT(*) OVER () (MySQL 8.0+ window function)
//   - WHERE workspace_id = ? added for multi-tenancy
func (r *MySQLBillingEventRepository) GetBillingEventListPageData(
	ctx context.Context,
...
func (r *MySQLBillingEventRepository) GetBillingEventItemPageData(
	ctx context.Context,
...
func (r *MySQLBillingEventRepository) ListBySubscription(
	ctx context.Context,
...
func (r *MySQLBillingEventRepository) ListByJobPhase(
	ctx context.Context,
...
// listByColumn is the shared list-with-equality helper.
// Dialect: $1 → ?, true → 1. Column name is allowlist-validated to prevent injection.
func (r *MySQLBillingEventRepository) listByColumn(
	...
	query := "SELECT * FROM `" + r.tableName + "` WHERE `" + column + "` = ? AND active = 1 ORDER BY date_created ASC"
	exec := r.dbOps.(executorProvider).GetExecutor(ctx)
	...
	for rows.Next() {
		...
		dataJSON, err := json.Marshal(mysqlCore.DenormalizeKeys(raw))
		if err != nil {
...
func (r *MySQLBillingEventRepository) SetStatus(
	ctx context.Context,
