...
func init() {
	registry.RegisterRepositoryFactory("postgresql", entityid.AssetRevaluation, func(conn any, tableName string) (any, error) {
		db, ok := conn.(*sql.DB)
		if !ok {
			return nil, fmt.Errorf("postgres asset_revaluation repository requires *sql.DB, got %T", conn)
		}
		dbOps := postgresCore.NewWorkspaceAwareOperations(db)
		return NewPostgresAssetRevaluationRepository(dbOps, tableName), nil
	})
}
...
// PostgresAssetRevaluationRepository implements asset_revaluation CRUD operations
// using PostgreSQL. AssetRevaluation rows are append-only: no business flow
// deletes or updates existing rows — the history forms the IAS 16 surplus ledger.
// Update/Delete delegates to dbOps for admin corrections only.
//
// Performance Index Recommendations:
//   - CREATE INDEX idx_asset_revaluation_asset_id ON asset_revaluation(asset_id)
//   - CREATE INDEX idx_asset_revaluation_revaluation_date ON asset_revaluation(revaluation_date)
type PostgresAssetRevaluationRepository struct {
	revaluationpb.UnimplementedAssetRevaluationDomainServiceServer
	dbOps     interfaces.DatabaseOperation
	tableName string
}

// NewPostgresAssetRevaluationRepository creates a new PostgreSQL asset_revaluation repository.
func NewPostgresAssetRevaluationRepository(dbOps interfaces.DatabaseOperation, tableName string) revaluationpb.AssetRevaluationDomainServiceServer {
	if tableName == "" {
		tableName = "asset_revaluation"
	}
	return &PostgresAssetRevaluationRepository{
		dbOps:     dbOps,
		tableName: tableName,
	}
}
...
func (r *PostgresAssetRevaluationRepository) CreateAssetRevaluation(ctx context.Context, req *revaluationpb.CreateAssetRevaluationRequest) (*revaluationpb.CreateAssetRevaluationResponse, error) {
	if req.Data == nil {
...
func (r *PostgresAssetRevaluationRepository) ReadAssetRevaluation(ctx context.Context, req *revaluationpb.ReadAssetRevaluationRequest) (*revaluationpb.ReadAssetRevaluationResponse, error) {
...
func (r *PostgresAssetRevaluationRepository) UpdateAssetRevaluation(ctx context.Context, req *revaluationpb.UpdateAssetRevaluationRequest) (*revaluationpb.UpdateAssetRevaluationResponse, error) {
...
func (r *PostgresAssetRevaluationRepository) DeleteAssetRevaluation(ctx context.Context, req *revaluationpb.DeleteAssetRevaluationRequest) (*revaluationpb.DeleteAssetRevaluationResponse, error) {
	if req.Data == nil || req.Data.Id == "" {
		return nil, fmt.Errorf("asset_revaluation ID is required")
	}
...
// ListAssetRevaluations lists asset_revaluation rows. The request may carry
// an asset_id filter to scope to a single asset's revaluation history.
func (r *PostgresAssetRevaluationRepository) ListAssetRevaluations(ctx context.Context, req *revaluationpb.ListAssetRevaluationsRequest) (*revaluationpb.ListAssetRevaluationsResponse, error) {
...
func (r *PostgresAssetRevaluationRepository) GetAssetRevaluationListPageData(
	ctx context.Context,
...
func (r *PostgresAssetRevaluationRepository) GetAssetRevaluationItemPageData(
	ctx context.Context,
...
// NewAssetRevaluationRepository creates a new PostgreSQL asset_revaluation repository (old-style constructor).
func NewAssetRevaluationRepository(db *sql.DB, tableName string) revaluationpb.AssetRevaluationDomainServiceServer {
	dbOps := postgresCore.NewWorkspaceAwareOperations(db)
	return NewPostgresAssetRevaluationRepository(dbOps, tableName)
}
