...
func init() {
	registry.RegisterRepositoryFactory("postgresql", entityid.AssetTransaction, func(conn any, tableName string) (any, error) {
		db, ok := conn.(*sql.DB)
		if !ok {
			return nil, fmt.Errorf("postgres asset_transaction repository requires *sql.DB, got %T", conn)
		}
		dbOps := postgresCore.NewWorkspaceAwareOperations(db)
		return NewPostgresAssetTransactionRepository(dbOps, tableName), nil
	})
}
...
// PostgresAssetTransactionRepository implements asset_transaction CRUD operations
// using PostgreSQL. AssetTransaction rows are append-only: no UpdateAssetTransaction
...
//   - CREATE INDEX idx_asset_transaction_depreciation_run_id ON asset_transaction(depreciation_run_id)
//   - CREATE INDEX idx_asset_transaction_revaluation_id ON asset_transaction(asset_revaluation_id) WHERE asset_revaluation_id IS NOT NULL
...
type PostgresAssetTransactionRepository struct {
	assettxpb.UnimplementedAssetTransactionDomainServiceServer
	dbOps     interfaces.DatabaseOperation
	tableName string
}

// NewPostgresAssetTransactionRepository creates a new PostgreSQL asset_transaction repository.
func NewPostgresAssetTransactionRepository(dbOps interfaces.DatabaseOperation, tableName string) assettxpb.AssetTransactionDomainServiceServer {
	if tableName == "" {
		tableName = "asset_transaction"
	}
	return &PostgresAssetTransactionRepository{
		dbOps:     dbOps,
		tableName: tableName,
	}
}
...
func (r *PostgresAssetTransactionRepository) CreateAssetTransaction(ctx context.Context, req *assettxpb.CreateAssetTransactionRequest) (*assettxpb.CreateAssetTransactionResponse, error) {
	if req.Data == nil {
...
func (r *PostgresAssetTransactionRepository) ReadAssetTransaction(ctx context.Context, req *assettxpb.ReadAssetTransactionRequest) (*assettxpb.ReadAssetTransactionResponse, error) {
...
// ListAssetTransactions lists asset_transaction rows using common PostgreSQL operations.
func (r *PostgresAssetTransactionRepository) ListAssetTransactions(ctx context.Context, req *assettxpb.ListAssetTransactionsRequest) (*assettxpb.ListAssetTransactionsResponse, error) {
...
func (r *PostgresAssetTransactionRepository) GetAssetTransactionListPageData(
	ctx context.Context,
...
func (r *PostgresAssetTransactionRepository) GetAssetTransactionItemPageData(
	ctx context.Context,
...
// NewAssetTransactionRepository creates a new PostgreSQL asset_transaction repository (old-style constructor).
func NewAssetTransactionRepository(db *sql.DB, tableName string) assettxpb.AssetTransactionDomainServiceServer {
	dbOps := postgresCore.NewWorkspaceAwareOperations(db)
	return NewPostgresAssetTransactionRepository(dbOps, tableName)
}
