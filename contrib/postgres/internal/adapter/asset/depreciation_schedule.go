...
func init() {
	registry.RegisterRepositoryFactory("postgresql", entityid.DepreciationSchedule, func(conn any, tableName string) (any, error) {
		db, ok := conn.(*sql.DB)
		if !ok {
			return nil, fmt.Errorf("postgres depreciation_schedule repository requires *sql.DB, got %T", conn)
		}
		dbOps := postgresCore.NewWorkspaceAwareOperations(db)
		return NewPostgresDepreciationScheduleRepository(dbOps, tableName), nil
	})
}
...
// PostgresDepreciationScheduleRepository implements depreciation_schedule CRUD
// operations using PostgreSQL.
//
// Performance Index Recommendations:
//   - CREATE INDEX idx_depreciation_schedule_asset_id ON depreciation_schedule(asset_id)
//   - CREATE INDEX idx_depreciation_schedule_depreciation_run_id ON depreciation_schedule(depreciation_run_id)
//   - CREATE UNIQUE INDEX idx_asset_transaction_depreciation_period ON asset_transaction(asset_id, depreciation_period_start_date) WHERE depreciation_run_id IS NOT NULL AND active = true
type PostgresDepreciationScheduleRepository struct {
	depschpb.UnimplementedDepreciationDomainServiceServer
	dbOps     interfaces.DatabaseOperation
	tableName string
}

// NewPostgresDepreciationScheduleRepository creates a new PostgreSQL depreciation_schedule repository.
func NewPostgresDepreciationScheduleRepository(dbOps interfaces.DatabaseOperation, tableName string) depschpb.DepreciationDomainServiceServer {
	if tableName == "" {
		tableName = "depreciation_schedule"
	}
	return &PostgresDepreciationScheduleRepository{
		dbOps:     dbOps,
		tableName: tableName,
	}
}
...
func (r *PostgresDepreciationScheduleRepository) CreateDepreciationSchedule(ctx context.Context, req *depschpb.CreateDepreciationScheduleRequest) (*depschpb.CreateDepreciationScheduleResponse, error) {
	if req.Data == nil {
...
func (r *PostgresDepreciationScheduleRepository) ReadDepreciationSchedule(ctx context.Context, req *depschpb.ReadDepreciationScheduleRequest) (*depschpb.ReadDepreciationScheduleResponse, error) {
...
// business-layer appends new rows rather than updating existing ones).
func (r *PostgresDepreciationScheduleRepository) UpdateDepreciationSchedule(ctx context.Context, req *depschpb.UpdateDepreciationScheduleRequest) (*depschpb.UpdateDepreciationScheduleResponse, error) {
...
func (r *PostgresDepreciationScheduleRepository) DeleteDepreciationSchedule(ctx context.Context, req *depschpb.DeleteDepreciationScheduleRequest) (*depschpb.DeleteDepreciationScheduleResponse, error) {
	if req.Data == nil || req.Data.Id == "" {
		return nil, fmt.Errorf("depreciation_schedule ID is required")
	}
...
// ListDepreciationSchedules lists depreciation_schedule rows.
func (r *PostgresDepreciationScheduleRepository) ListDepreciationSchedules(ctx context.Context, req *depschpb.ListDepreciationSchedulesRequest) (*depschpb.ListDepreciationSchedulesResponse, error) {
...
func (r *PostgresDepreciationScheduleRepository) GetDepreciationScheduleListPageData(
	ctx context.Context,
...
func (r *PostgresDepreciationScheduleRepository) GetDepreciationScheduleItemPageData(
	ctx context.Context,
...
// NewDepreciationScheduleRepository creates a new PostgreSQL depreciation_schedule repository (old-style constructor).
func NewDepreciationScheduleRepository(db *sql.DB, tableName string) depschpb.DepreciationDomainServiceServer {
	dbOps := postgresCore.NewWorkspaceAwareOperations(db)
	return NewPostgresDepreciationScheduleRepository(dbOps, tableName)
}
