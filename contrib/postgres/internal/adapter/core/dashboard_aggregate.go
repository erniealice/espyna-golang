...
// `return 0, nil`. A transient DB fault then silently rendered a dashboard
// full of zeros that looked like real data. This helper makes that pattern
// structurally impossible by funnelling every scalar dashboard metric through
// a SINGLE round-trip whose error is propagated honestly:
//
