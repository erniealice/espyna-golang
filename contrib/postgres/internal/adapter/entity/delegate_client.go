...
// delegateClientSortableSQLCols lists the SQL column names that are safe to sort
// by in GetDelegateClientListPageData. The query uses direct ORDER BY interpolation
...
var delegateClientSortableSQLCols = []string{
	"date_created",
	"date_modified",
	"delegate_id",
	"client_id",
...
// delegateClientViewToSQLColMap translates view-facing sort column keys to SQL
// column names. Columns absent from the map pass through unchanged.
var delegateClientViewToSQLColMap = map[string]string{}

func init() {
	registry.RegisterRepositoryFactory("postgresql", entityid.DelegateClient, func(conn any, tableName string) (any, error) {
		db, ok := conn.(*sql.DB)
		if !ok {
...
func (r *PostgresDelegateClientRepository) GetDelegateClientListPageData(ctx context.Context, req *delegateclientpb.GetDelegateClientListPageDataRequest) (*delegateclientpb.GetDelegateClientListPageDataResponse, error) {
	...
	if mapped, ok := delegateClientViewToSQLColMap[sortField]; ok {
		sortField = mapped
	}
	...
	if sortField != "" && !slices.Contains(delegateClientSortableSQLCols, sortField) {
