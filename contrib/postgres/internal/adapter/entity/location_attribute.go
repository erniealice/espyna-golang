...
func init() {
	registry.RegisterRepositoryFactory("postgresql", entityid.LocationAttribute, func(conn any, tableName string) (any, error) {
		db, ok := conn.(*sql.DB)
		if !ok {
...
// locationAttributeSortableSQLCols is the fail-closed sort whitelist for
// GetLocationAttributeListPageData. Only columns projected by the CTE SELECT are
// included so ORDER BY can never reference an unprojected/injected identifier.
var locationAttributeSortableSQLCols = []string{
	"id", "location_id", "key", "value", "active", "date_created", "date_modified",
}
...
func (r *PostgresLocationAttributeRepository) GetLocationAttributeListPageData(ctx context.Context, req *locationattributepb.GetLocationAttributeListPageDataRequest) (*locationattributepb.GetLocationAttributeListPageDataResponse, error) {
	...
	orderByClause, err := postgresCore.BuildOrderBy(locationAttributeSortableSQLCols, req.GetSort(), "date_created DESC")
	if err != nil {
		return nil, err
	}
