...
// Compile-time assertions: every postgres equity-dashboard repo MUST satisfy
...
//
// The equity dashboard composition root performs runtime type assertions
...
// `ledger.EquityAccountSlice` instead of `equitydash.EquityAccountSlice`.
//
...
// (Admin shipped this guard rail first; Location, Ledger, Treasury,
// Payroll, etc. each ship their own). Per Wave B P1.C.4.
var (
	_ equitydash.EquityAccountDashboardRepository     = (*PostgresEquityAccountRepository)(nil)
	_ equitydash.EquityTransactionDashboardRepository = (*PostgresEquityTransactionRepository)(nil)
)
