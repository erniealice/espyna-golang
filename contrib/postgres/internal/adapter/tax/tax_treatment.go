...
func init() {
	registry.RegisterRepositoryFactory("postgresql", entityid.TaxTreatment, func(conn any, tableName string) (any, error) {
		db, ok := conn.(*sql.DB)
		if !ok {
			return nil, fmt.Errorf("postgres tax_treatment repository requires *sql.DB, got %T", conn)
		}
		dbOps := postgresCore.NewWorkspaceAwareOperations(db)
		return NewPostgresTaxTreatmentRepository(dbOps, tableName), nil
	})
}

// PostgresTaxTreatmentRepository implements tax_treatment read operations using PostgreSQL.
type PostgresTaxTreatmentRepository struct {
	taxtreatmentpb.UnimplementedTaxTreatmentDomainServiceServer
	dbOps     interfaces.DatabaseOperation
	tableName string
}

// NewPostgresTaxTreatmentRepository creates a new PostgreSQL tax_treatment repository.
func NewPostgresTaxTreatmentRepository(dbOps interfaces.DatabaseOperation, tableName string) taxtreatmentpb.TaxTreatmentDomainServiceServer {
	if tableName == "" {
		tableName = entityid.TaxTreatment
	}
	return &PostgresTaxTreatmentRepository{
		dbOps:     dbOps,
		tableName: tableName,
	}
}
...
func (r *PostgresTaxTreatmentRepository) ReadTaxTreatment(ctx context.Context, req *taxtreatmentpb.ReadTaxTreatmentRequest) (*taxtreatmentpb.ReadTaxTreatmentResponse, error) {
...
func (r *PostgresTaxTreatmentRepository) ListTaxTreatments(ctx context.Context, req *taxtreatmentpb.ListTaxTreatmentsRequest) (*taxtreatmentpb.ListTaxTreatmentsResponse, error) {
