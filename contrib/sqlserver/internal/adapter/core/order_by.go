...
//
// It mirrors contrib/postgres/internal/adapter/core.BuildOrderBy and
// contrib/mysql/internal/adapter/core.BuildOrderBy with one mechanical
// difference: identifiers are square-bracket quoted (T-SQL) instead of
// double-quoted (postgres) / backtick-quoted (MySQL). The quoting is delegated
// to SQLServerDialect.QuoteIdent so the logic stays dialect-driven.
//
...
//
// Behavior:
//   - sort nil/empty (no fields, or first field blank) → "ORDER BY " + fallback
//     verbatim (author-controlled constant, never caller-controlled).
...
// whitelist-validated and bracket-quoted PER DOT-COMPONENT, and the direction is
// enum-derived.
//
// DETERMINISTIC-PAGINATION TIEBREAKER (parity with postgres/mysql): the emitted
// clause always terminates in a UNIQUE final key so the OFFSET/FETCH pages these
// hand-written list adapters produce are stable, disjoint, and complete even when
// rows TIE on the leading sort key (e.g. many rows bulk-created under a single
// date_created). Without a unique final key SQL Server may return tied rows in a
// different order per fetch, silently dropping some rows from every page and
...
// (`SELECT ... FROM enriched ORDER BY ...`) whose sole exposed id column resolves
// unambiguously. A caller whose ORDER BY sits on a JOINED outer query (two+
// relations each carrying `id` directly in scope) must pass its primary table's
// alias-qualified id (e.g. "pr.id") so the appended key is unambiguous — an empty
// string keeps the "id" default. Direction is always ASC: stability needs a
...
// would silently defeat the uniqueness invariant) and emitted bracket-quoted PER
// COMPONENT ([pr].[id], never the single identifier [pr.id] in one bracket pair).
//
// The variadic tiebreaker keeps every pre-existing three-argument call site
// source-compatible.
...
// returned form is bracket-quoted per component ([pr].[id]) so it is safe to
...
// on commas OUTSIDE brackets (so a column literally named [x,id] is not torn into
// a phantom id segment), and each segment's leading path token has its FINAL
...
// square-bracket-quoted region. Unlike postgres (") and MySQL (`), T-SQL uses a
// DISTINCT open/close pair: a region starts at '[' and ends at the first ']' that
// is not doubled. Per T-SQL's rule a doubled ']' inside a quoted region is an
// escaped bracket, not a region close. Used to walk ORDER BY commas and dotted
...
// segment — everything up to the first whitespace OUTSIDE brackets (which strips
// the trailing ASC/DESC options).
...
// per-component bracket-quoted) path token, unquoted and lowercased — e.g.
...
// quoteSortIdent bracket-quotes a whitelisted SQL Server sort column so it is
// interpolated safely. It quotes PER DOT-SEPARATED COMPONENT — a qualified column
// `alias.col` becomes [alias].[col] (one bracket pair per component), NOT the
// single identifier [alias.col] in one pair (which references a nonexistent column
// literally named "alias.col" and breaks every alias-qualified whitelist entry such
// as payroll_run's `pr.date_created`). A bare column becomes [col]. Any embedded
// closing bracket is escaped per T-SQL's rule of doubling it (via
// SQLServerDialect.QuoteIdent). The column is already whitelist-checked; this is
// defense in depth.
