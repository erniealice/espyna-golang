...
func init() {
	registry.RegisterRepositoryFactory("sqlserver", entityid.Attachment, func(conn any, tableName string) (any, error) {
		db, ok := conn.(*sql.DB)
		if !ok {
			return nil, fmt.Errorf("sqlserver attachment repository requires *sql.DB, got %T", conn)
		}
		dbOps := sqlserverCore.NewWorkspaceAwareOperations(db)
		return NewSQLServerAttachmentRepository(dbOps, tableName), nil
	})
}

// SQLServerAttachmentRepository implements attachment CRUD operations using SQL Server.
//
...
type SQLServerAttachmentRepository struct {
	attachmentpb.UnimplementedAttachmentDomainServiceServer
	dbOps     interfaces.DatabaseOperation
	tableName string
}

// NewSQLServerAttachmentRepository creates a new SQL Server attachment repository.
func NewSQLServerAttachmentRepository(dbOps interfaces.DatabaseOperation, tableName string) attachmentpb.AttachmentDomainServiceServer {
	if tableName == "" {
		tableName = "attachment"
	}
	return &SQLServerAttachmentRepository{
		dbOps:     dbOps,
		tableName: tableName,
	}
}
...
func (r *SQLServerAttachmentRepository) CreateAttachment(ctx context.Context, req *attachmentpb.CreateAttachmentRequest) (*attachmentpb.CreateAttachmentResponse, error) {
	if req.Data == nil {
...
func (r *SQLServerAttachmentRepository) ReadAttachment(ctx context.Context, req *attachmentpb.ReadAttachmentRequest) (*attachmentpb.ReadAttachmentResponse, error) {
...
func (r *SQLServerAttachmentRepository) UpdateAttachment(ctx context.Context, req *attachmentpb.UpdateAttachmentRequest) (*attachmentpb.UpdateAttachmentResponse, error) {
	if req.Data == nil || req.Data.Id == "" {
		return nil, fmt.Errorf("attachment ID is required")
	}
...
// DeleteAttachment soft-deletes an attachment record.
func (r *SQLServerAttachmentRepository) DeleteAttachment(ctx context.Context, req *attachmentpb.DeleteAttachmentRequest) (*attachmentpb.DeleteAttachmentResponse, error) {
	if req.Data == nil || req.Data.Id == "" {
		return nil, fmt.Errorf("attachment ID is required")
	}
...
func (r *SQLServerAttachmentRepository) ListAttachments(ctx context.Context, req *attachmentpb.ListAttachmentsRequest) (*attachmentpb.ListAttachmentsResponse, error) {
...
// SQL Server datetime2 columns need time.Time, not raw millis.
...
// NewAttachmentRepository creates a new SQL Server attachment repository (old-style constructor).
func NewAttachmentRepository(db *sql.DB, tableName string) attachmentpb.AttachmentDomainServiceServer {
	dbOps := sqlserverCore.NewWorkspaceAwareOperations(db)
	return NewSQLServerAttachmentRepository(dbOps, tableName)
}
