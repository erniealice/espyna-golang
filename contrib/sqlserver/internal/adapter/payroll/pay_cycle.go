...
func init() {
	registry.RegisterRepositoryFactory("sqlserver", entityid.PayCycle, func(conn any, tableName string) (any, error) {
		db, ok := conn.(*sql.DB)
		if !ok {
			return nil, fmt.Errorf("sqlserver pay_cycle repository requires *sql.DB, got %T", conn)
		}
		dbOps := sqlserverCore.NewWorkspaceAwareOperations(db)
		return NewSQLServerPayCycleRepository(dbOps, tableName), nil
	})
}

// SQLServerPayCycleRepository implements pay cycle CRUD operations using SQL Server.
type SQLServerPayCycleRepository struct {
	paycyclepb.UnimplementedPayCycleDomainServiceServer
	dbOps     interfaces.DatabaseOperation
	tableName string
}

// NewSQLServerPayCycleRepository creates a new SQL Server pay cycle repository.
func NewSQLServerPayCycleRepository(dbOps interfaces.DatabaseOperation, tableName string) paycyclepb.PayCycleDomainServiceServer {
	if tableName == "" {
		tableName = "pay_cycle"
	}
	return &SQLServerPayCycleRepository{
		dbOps:     dbOps,
		tableName: tableName,
	}
}
...
func (r *SQLServerPayCycleRepository) CreatePayCycle(ctx context.Context, req *paycyclepb.CreatePayCycleRequest) (*paycyclepb.CreatePayCycleResponse, error) {
	if req.Data == nil {
...
func (r *SQLServerPayCycleRepository) ReadPayCycle(ctx context.Context, req *paycyclepb.ReadPayCycleRequest) (*paycyclepb.ReadPayCycleResponse, error) {
...
func (r *SQLServerPayCycleRepository) UpdatePayCycle(ctx context.Context, req *paycyclepb.UpdatePayCycleRequest) (*paycyclepb.UpdatePayCycleResponse, error) {
	if req.Data == nil || req.Data.Id == "" {
		return nil, fmt.Errorf("pay cycle ID is required")
	}
	jsonData, err := protojson.Marshal(req.Data)
...
func (r *SQLServerPayCycleRepository) DeletePayCycle(ctx context.Context, req *paycyclepb.DeletePayCycleRequest) (*paycyclepb.DeletePayCycleResponse, error) {
	if req.Data == nil || req.Data.Id == "" {
		return nil, fmt.Errorf("pay cycle ID is required")
	}
...
func (r *SQLServerPayCycleRepository) ListPayCycles(ctx context.Context, req *paycyclepb.ListPayCyclesRequest) (*paycyclepb.ListPayCyclesResponse, error) {
...
func (r *SQLServerPayCycleRepository) GetPayCycleListPageData(
	ctx context.Context,
...
func (r *SQLServerPayCycleRepository) GetPayCycleItemPageData(
	ctx context.Context,
