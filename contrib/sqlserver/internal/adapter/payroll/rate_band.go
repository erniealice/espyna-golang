...
func init() {
	registry.RegisterRepositoryFactory("sqlserver", entityid.RateBand, func(conn any, tableName string) (any, error) {
		db, ok := conn.(*sql.DB)
		if !ok {
			return nil, fmt.Errorf("sqlserver rate_band repository requires *sql.DB, got %T", conn)
		}
		dbOps := sqlserverCore.NewWorkspaceAwareOperations(db)
		return NewSQLServerRateBandRepository(dbOps, tableName), nil
	})
}

// SQLServerRateBandRepository implements rate band CRUD operations using SQL Server.
type SQLServerRateBandRepository struct {
	ratebandpb.UnimplementedRateBandDomainServiceServer
	dbOps     interfaces.DatabaseOperation
	tableName string
}

// NewSQLServerRateBandRepository creates a new SQL Server rate band repository.
func NewSQLServerRateBandRepository(dbOps interfaces.DatabaseOperation, tableName string) ratebandpb.RateBandDomainServiceServer {
	if tableName == "" {
		tableName = "rate_band"
	}
	return &SQLServerRateBandRepository{
		dbOps:     dbOps,
		tableName: tableName,
	}
}
...
func (r *SQLServerRateBandRepository) CreateRateBand(ctx context.Context, req *ratebandpb.CreateRateBandRequest) (*ratebandpb.CreateRateBandResponse, error) {
	if req.Data == nil {
...
func (r *SQLServerRateBandRepository) ReadRateBand(ctx context.Context, req *ratebandpb.ReadRateBandRequest) (*ratebandpb.ReadRateBandResponse, error) {
...
func (r *SQLServerRateBandRepository) UpdateRateBand(ctx context.Context, req *ratebandpb.UpdateRateBandRequest) (*ratebandpb.UpdateRateBandResponse, error) {
	if req.Data == nil || req.Data.Id == "" {
		return nil, fmt.Errorf("rate band ID is required")
	}
	jsonData, err := protojson.Marshal(req.Data)
...
func (r *SQLServerRateBandRepository) DeleteRateBand(ctx context.Context, req *ratebandpb.DeleteRateBandRequest) (*ratebandpb.DeleteRateBandResponse, error) {
	if req.Data == nil || req.Data.Id == "" {
		return nil, fmt.Errorf("rate band ID is required")
	}
...
func (r *SQLServerRateBandRepository) ListRateBands(ctx context.Context, req *ratebandpb.ListRateBandsRequest) (*ratebandpb.ListRateBandsResponse, error) {
...
func (r *SQLServerRateBandRepository) GetRateBandListPageData(
	ctx context.Context,
...
func (r *SQLServerRateBandRepository) GetRateBandItemPageData(
	ctx context.Context,
