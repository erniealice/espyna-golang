...
func init() {
	registry.RegisterRepositoryFactory("sqlserver", entityid.DeferredRevenue, func(conn any, tableName string) (any, error) {
		db, ok := conn.(*sql.DB)
		if !ok {
			return nil, fmt.Errorf("sqlserver deferred_revenue repository requires *sql.DB, got %T", conn)
		}
		dbOps := sqlserverCore.NewWorkspaceAwareOperations(db)
		return NewSQLServerDeferredRevenueRepository(dbOps, tableName), nil
	})
}

// SQLServerDeferredRevenueRepository implements deferred_revenue CRUD using SQL Server.
type SQLServerDeferredRevenueRepository struct {
	deferredrevenuepb.UnimplementedDeferredRevenueDomainServiceServer
	dbOps     interfaces.DatabaseOperation
	tableName string
}

// NewSQLServerDeferredRevenueRepository creates a new SQL Server deferred revenue repository.
func NewSQLServerDeferredRevenueRepository(dbOps interfaces.DatabaseOperation, tableName string) deferredrevenuepb.DeferredRevenueDomainServiceServer {
	if tableName == "" {
		tableName = "deferred_revenue"
	}
	return &SQLServerDeferredRevenueRepository{
		dbOps:     dbOps,
		tableName: tableName,
	}
}
...
func (r *SQLServerDeferredRevenueRepository) CreateDeferredRevenue(ctx context.Context, req *deferredrevenuepb.CreateDeferredRevenueRequest) (*deferredrevenuepb.CreateDeferredRevenueResponse, error) {
	if req.Data == nil {
...
func (r *SQLServerDeferredRevenueRepository) ReadDeferredRevenue(ctx context.Context, req *deferredrevenuepb.ReadDeferredRevenueRequest) (*deferredrevenuepb.ReadDeferredRevenueResponse, error) {
...
func (r *SQLServerDeferredRevenueRepository) UpdateDeferredRevenue(ctx context.Context, req *deferredrevenuepb.UpdateDeferredRevenueRequest) (*deferredrevenuepb.UpdateDeferredRevenueResponse, error) {
	if req.Data == nil || req.Data.Id == "" {
		return nil, fmt.Errorf("deferred revenue ID is required")
	}
	jsonData, err := protojson.Marshal(req.Data)
...
func (r *SQLServerDeferredRevenueRepository) DeleteDeferredRevenue(ctx context.Context, req *deferredrevenuepb.DeleteDeferredRevenueRequest) (*deferredrevenuepb.DeleteDeferredRevenueResponse, error) {
	if req.Data == nil || req.Data.Id == "" {
		return nil, fmt.Errorf("deferred revenue ID is required")
	}
...
func (r *SQLServerDeferredRevenueRepository) ListDeferredRevenues(ctx context.Context, req *deferredrevenuepb.ListDeferredRevenuesRequest) (*deferredrevenuepb.ListDeferredRevenuesResponse, error) {
...
// GetDeferredRevenueListPageData retrieves deferred revenues with pagination, filtering, sorting, and search.
//
// SQL Server differences from the postgres gold standard:
//   - @p1 = searchPattern, @p2 = offset, @p3 = limit.
//   - ILIKE → LIKE.
//   - OFFSET/FETCH pagination.
//   - No postgres-specific cast syntax.
func (r *SQLServerDeferredRevenueRepository) GetDeferredRevenueListPageData(
	ctx context.Context,
	...
	// No ::text cast; ILIKE → LIKE; LIMIT/OFFSET → OFFSET/FETCH.
	query := fmt.Sprintf(`
		WITH enriched AS (
			SELECT
				dr.id,
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			&id, &dateCreated, &dateModified, &active, &description,
			&customerName, &totalAmount, &recognizedAmount, &remainingAmount,
			&startDate, &endDate, &recognitionMonths, &statusStr,
			&liabilityAccountID, &revenueAccountID, &total,
		); err != nil {
...
func (r *SQLServerDeferredRevenueRepository) GetDeferredRevenueItemPageData(
	ctx context.Context,
...
// NewDeferredRevenueRepository creates a new SQL Server deferred revenue repository (old-style constructor).
func NewDeferredRevenueRepository(db *sql.DB, tableName string) deferredrevenuepb.DeferredRevenueDomainServiceServer {
	dbOps := sqlserverCore.NewWorkspaceAwareOperations(db)
	return NewSQLServerDeferredRevenueRepository(dbOps, tableName)
}
