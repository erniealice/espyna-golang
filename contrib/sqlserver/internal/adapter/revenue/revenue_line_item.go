...
func init() {
	registry.RegisterRepositoryFactory("sqlserver", entityid.RevenueLineItem, func(conn any, tableName string) (any, error) {
		db, ok := conn.(*sql.DB)
		if !ok {
			return nil, fmt.Errorf("sqlserver revenue_line_item repository requires *sql.DB, got %T", conn)
		}
		dbOps := sqlserverCore.NewWorkspaceAwareOperations(db)
		return NewSQLServerRevenueLineItemRepository(dbOps, tableName), nil
	})
}

// SQLServerRevenueLineItemRepository implements revenue_line_item CRUD using SQL Server.
type SQLServerRevenueLineItemRepository struct {
	revenuelineitempb.UnimplementedRevenueLineItemDomainServiceServer
	dbOps     interfaces.DatabaseOperation
	tableName string
}

// NewSQLServerRevenueLineItemRepository creates a new SQL Server revenue line item repository.
func NewSQLServerRevenueLineItemRepository(dbOps interfaces.DatabaseOperation, tableName string) revenuelineitempb.RevenueLineItemDomainServiceServer {
	if tableName == "" {
		tableName = "revenue_line_item"
	}
	return &SQLServerRevenueLineItemRepository{
		dbOps:     dbOps,
		tableName: tableName,
	}
}
...
func (r *SQLServerRevenueLineItemRepository) CreateRevenueLineItem(ctx context.Context, req *revenuelineitempb.CreateRevenueLineItemRequest) (*revenuelineitempb.CreateRevenueLineItemResponse, error) {
	if req.Data == nil {
	...
	return &revenuelineitempb.CreateRevenueLineItemResponse{Data: []*revenuelineitempb.RevenueLineItem{lineItem}}, nil
}

// ReadRevenueLineItem retrieves a revenue line item by ID.
func (r *SQLServerRevenueLineItemRepository) ReadRevenueLineItem(ctx context.Context, req *revenuelineitempb.ReadRevenueLineItemRequest) (*revenuelineitempb.ReadRevenueLineItemResponse, error) {
	...
	return &revenuelineitempb.ReadRevenueLineItemResponse{Data: []*revenuelineitempb.RevenueLineItem{lineItem}}, nil
}

// UpdateRevenueLineItem updates a revenue line item.
func (r *SQLServerRevenueLineItemRepository) UpdateRevenueLineItem(ctx context.Context, req *revenuelineitempb.UpdateRevenueLineItemRequest) (*revenuelineitempb.UpdateRevenueLineItemResponse, error) {
	if req.Data == nil || req.Data.Id == "" {
		return nil, fmt.Errorf("revenue line item ID is required")
	}
	jsonData, err := protojson.Marshal(req.Data)
	...
	return &revenuelineitempb.UpdateRevenueLineItemResponse{Data: []*revenuelineitempb.RevenueLineItem{lineItem}}, nil
}

// DeleteRevenueLineItem soft-deletes a revenue line item.
func (r *SQLServerRevenueLineItemRepository) DeleteRevenueLineItem(ctx context.Context, req *revenuelineitempb.DeleteRevenueLineItemRequest) (*revenuelineitempb.DeleteRevenueLineItemResponse, error) {
	if req.Data == nil || req.Data.Id == "" {
		return nil, fmt.Errorf("revenue line item ID is required")
	}
...
func (r *SQLServerRevenueLineItemRepository) ListRevenueLineItems(ctx context.Context, req *revenuelineitempb.ListRevenueLineItemsRequest) (*revenuelineitempb.ListRevenueLineItemsResponse, error) {
...
// GetRevenueLineItemListPageData retrieves revenue line items with pagination, filtering, sorting, and search.
//
// SQL Server differences from the postgres gold standard:
//   - @p1 = searchPattern, @p2 = offset, @p3 = limit.
//   - ILIKE → LIKE.
//   - OFFSET/FETCH pagination (ORDER BY is mandatory).
//   - COUNT(*) OVER () retained.
func (r *SQLServerRevenueLineItemRepository) GetRevenueLineItemListPageData(
	ctx context.Context,
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			&id, &dateCreated, &dateModified, &active, &revenueID, &productID,
			&description, &quantity, &unitPrice, &totalPrice, &notes, &lineItemType,
			&inventoryItemID, &inventorySerialID, &productPricePlanID, &priceProductID,
			&revenueName, &productName, &total,
		); err != nil {
		...
		_ = productPricePlanID // removed from proto
...
// GetRevenueLineItemItemPageData retrieves a single revenue line item with enriched data.
func (r *SQLServerRevenueLineItemRepository) GetRevenueLineItemItemPageData(
	ctx context.Context,
	...
	exec := r.dbOps.(executorProvider).GetExecutor(ctx)
	row := exec.QueryRowContext(ctx, query, req.RevenueLineItemId)
...
// NewRevenueLineItemRepository creates a new SQL Server revenue line item repository (old-style constructor).
func NewRevenueLineItemRepository(db *sql.DB, tableName string) revenuelineitempb.RevenueLineItemDomainServiceServer {
	dbOps := sqlserverCore.NewWorkspaceAwareOperations(db)
	return NewSQLServerRevenueLineItemRepository(dbOps, tableName)
}
