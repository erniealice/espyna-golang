...
func init() {
	registry.RegisterRepositoryFactory("sqlserver", entityid.BillingEvent, func(conn any, tableName string) (any, error) {
		db, ok := conn.(*sql.DB)
		if !ok {
			return nil, fmt.Errorf("sqlserver billing_event repository requires *sql.DB, got %T", conn)
		}
		dbOps := sqlserverCore.NewWorkspaceAwareOperations(db)
		return NewSQLServerBillingEventRepository(dbOps, tableName), nil
	})
}

// SQLServerBillingEventRepository implements billing_event CRUD operations using SQL Server.
type SQLServerBillingEventRepository struct {
...
// NewSQLServerBillingEventRepository creates a new SQL Server billing_event repository.
func NewSQLServerBillingEventRepository(dbOps interfaces.DatabaseOperation, tableName string) pb.BillingEventDomainServiceServer {
	if tableName == "" {
		tableName = "billing_event"
	}
	return &SQLServerBillingEventRepository{
		dbOps:     dbOps,
		db:        nil, // unused; queries run via executorProvider
		tableName: tableName,
	}
}
...
func (r *SQLServerBillingEventRepository) getDB(ctx context.Context) (sqlexec.DBExecutor, error) {
	...
	return ep.GetExecutor(ctx), nil
}

// CreateBillingEvent inserts a new billing_event row.
func (r *SQLServerBillingEventRepository) CreateBillingEvent(ctx context.Context, req *pb.CreateBillingEventRequest) (*pb.CreateBillingEventResponse, error) {
	if req.Data == nil {
...
func (r *SQLServerBillingEventRepository) ReadBillingEvent(ctx context.Context, req *pb.ReadBillingEventRequest) (*pb.ReadBillingEventResponse, error) {
...
func (r *SQLServerBillingEventRepository) UpdateBillingEvent(ctx context.Context, req *pb.UpdateBillingEventRequest) (*pb.UpdateBillingEventResponse, error) {
	if req.Data == nil || req.Data.Id == "" {
		return nil, fmt.Errorf("billing event ID is required")
	}
...
func (r *SQLServerBillingEventRepository) DeleteBillingEvent(ctx context.Context, req *pb.DeleteBillingEventRequest) (*pb.DeleteBillingEventResponse, error) {
	if req.Data == nil || req.Data.Id == "" {
		return nil, fmt.Errorf("billing event ID is required")
	}
...
func (r *SQLServerBillingEventRepository) ListBillingEvents(ctx context.Context, req *pb.ListBillingEventsRequest) (*pb.ListBillingEventsResponse, error) {
...
func (r *SQLServerBillingEventRepository) GetBillingEventListPageData(
	...
	for rows.Next() {
		...
		dataJSON, _ := json.Marshal(sqlserverCore.DenormalizeKeys(raw))
		ev := &pb.BillingEvent{}
...
func (r *SQLServerBillingEventRepository) GetBillingEventItemPageData(
	ctx context.Context,
...
func (r *SQLServerBillingEventRepository) ListBySubscription(
	ctx context.Context,
...
func (r *SQLServerBillingEventRepository) ListByJobPhase(
	ctx context.Context,
...
//   - $1 → @p1.
//   - active = true → active = 1.
//   - LIMIT 1 not used here; ORDER BY is required in SQL Server regardless
//     (omitting ORDER BY on a paginated query would require OFFSET/FETCH,
//     but this is an unbounded query so none is needed).
func (r *SQLServerBillingEventRepository) listByColumn(
	...
	// SQL Server: $1 → @p1; active = true → active = 1.
	query := `SELECT * FROM ` + r.tableName + ` WHERE ` + column + ` = @p1 AND active = 1 ORDER BY date_created ASC`

	exec := r.dbOps.(executorProvider).GetExecutor(ctx)
	rows, err := exec.QueryContext(ctx, query, value)
	if err != nil {
	...
	for rows.Next() {
		...
		dataJSON, err := json.Marshal(sqlserverCore.DenormalizeKeys(raw))
		if err != nil {
...
func (r *SQLServerBillingEventRepository) SetStatus(
	ctx context.Context,
