...
// SQLServerPlanRepository implements plan CRUD operations using SQL Server.
type SQLServerPlanRepository struct {
...
func init() {
	registry.RegisterRepositoryFactory("sqlserver", entityid.Plan, func(conn any, tableName string) (any, error) {
		db, ok := conn.(*sql.DB)
		if !ok {
			return nil, fmt.Errorf("sqlserver plan repository requires *sql.DB, got %T", conn)
		}
		dbOps := sqlserverCore.NewWorkspaceAwareOperations(db)
		return NewSQLServerPlanRepository(dbOps, tableName), nil
	})
}

// NewSQLServerPlanRepository creates a new SQL Server plan repository.
func NewSQLServerPlanRepository(dbOps interfaces.DatabaseOperation, tableName string) planpb.PlanDomainServiceServer {
	if tableName == "" {
		tableName = "plan"
	}
	return &SQLServerPlanRepository{
		dbOps:     dbOps,
		tableName: tableName,
	}
}

// CreatePlan creates a new plan using common SQL Server operations.
func (r *SQLServerPlanRepository) CreatePlan(ctx context.Context, req *planpb.CreatePlanRequest) (*planpb.CreatePlanResponse, error) {
	if req.Data == nil {
...
// ReadPlan retrieves a plan using common SQL Server operations.
func (r *SQLServerPlanRepository) ReadPlan(ctx context.Context, req *planpb.ReadPlanRequest) (*planpb.ReadPlanResponse, error) {
...
// UpdatePlan updates a plan using common SQL Server operations.
func (r *SQLServerPlanRepository) UpdatePlan(ctx context.Context, req *planpb.UpdatePlanRequest) (*planpb.UpdatePlanResponse, error) {
	if req.Data == nil || req.Data.Id == nil || *req.Data.Id == "" {
		return nil, fmt.Errorf("plan ID is required")
	}
...
// DeletePlan hard-deletes a plan (catalog entity; FK RESTRICT prevents deletion when referenced).
func (r *SQLServerPlanRepository) DeletePlan(ctx context.Context, req *planpb.DeletePlanRequest) (*planpb.DeletePlanResponse, error) {
	if req.Data == nil || req.Data.Id == nil || *req.Data.Id == "" {
		return nil, fmt.Errorf("plan ID is required")
	}
...
// ListPlans lists plans using common SQL Server operations.
func (r *SQLServerPlanRepository) ListPlans(ctx context.Context, req *planpb.ListPlansRequest) (*planpb.ListPlansResponse, error) {
...
// GetPlanListPageData retrieves a paginated, filtered, sorted, and searchable list of plans
// with adjacent plan_location relationships. Delegates the field-agnostic plan loading to
// the generic List path (dbOps.List + protojson DiscardUnknown round-trip).
func (r *SQLServerPlanRepository) GetPlanListPageData(ctx context.Context, req *planpb.GetPlanListPageDataRequest) (*planpb.GetPlanListPageDataResponse, error) {
	if req == nil {
		req = &planpb.GetPlanListPageDataRequest{}
	}

	filters := mergeActiveFilterSQL(req.GetFilters(), true)
	params := &interfaces.ListParams{
...
// GetPlanItemPageData retrieves a single plan with adjacent plan_location relationships.
func (r *SQLServerPlanRepository) GetPlanItemPageData(ctx context.Context, req *planpb.GetPlanItemPageDataRequest) (*planpb.GetPlanItemPageDataResponse, error) {
...
// planFromMapSQL converts a plan row map into a Plan proto via protojson round-trip.
func planFromMapSQL(row map[string]any) (*planpb.Plan, error) {
	resultJSON, err := json.Marshal(sqlserverCore.DenormalizeKeys(row))
	if err != nil {
		return nil, err
	}
	plan := &planpb.Plan{}
...
// mergeActiveFilterSQL returns a FilterRequest with `active = <value>` enforced.
func mergeActiveFilterSQL(in *commonpb.FilterRequest, active bool) *commonpb.FilterRequest {
...
// loadPlanLocationsByPlanIDs returns a map keyed by plan_id of attached, active PlanLocation rows.
func (r *SQLServerPlanRepository) loadPlanLocationsByPlanIDs(ctx context.Context, planIDs []string) (map[string][]*planlocationpb.PlanLocation, error) {
	...
	for _, row := range plLinks.Data {
		js, err := json.Marshal(sqlserverCore.DenormalizeKeys(row))
		if err != nil {
			continue
		}
...
// SearchPlansByName searches active plans by name using LIKE (SQL Server CI).
//
// SQL Server differences:
//   - $1/$2 → @p1/@p2.
//   - ILIKE → LIKE.
//   - active = true → active = 1.
//   - LIMIT $2 → OFFSET 0 ROWS FETCH NEXT @p2 ROWS ONLY (ORDER BY required).
func (r *SQLServerPlanRepository) SearchPlansByName(ctx context.Context, req *planpb.SearchPlansByNameRequest) (*planpb.SearchPlansByNameResponse, error) {
	if req == nil {
	...
	// SQL Server: LIKE (not ILIKE); OFFSET/FETCH instead of LIMIT; [plan] bracket quoting
...
// NewPlanRepository creates a new SQL Server plan repository (old-style constructor).
func NewPlanRepository(db *sql.DB, tableName string) planpb.PlanDomainServiceServer {
	dbOps := sqlserverCore.NewWorkspaceAwareOperations(db)
	return NewSQLServerPlanRepository(dbOps, tableName)
}
