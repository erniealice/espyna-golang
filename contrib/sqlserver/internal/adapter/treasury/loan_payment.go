...
func init() {
	registry.RegisterRepositoryFactory("sqlserver", entityid.LoanPayment, func(conn any, tableName string) (any, error) {
		db, ok := conn.(*sql.DB)
		if !ok {
			return nil, fmt.Errorf("sqlserver loan_payment repository requires *sql.DB, got %T", conn)
		}
		dbOps := sqlserverCore.NewWorkspaceAwareOperations(db)
		return NewSQLServerLoanPaymentRepository(dbOps, tableName), nil
	})
}

// SQLServerLoanPaymentRepository implements loan_payment CRUD operations using SQL Server.
type SQLServerLoanPaymentRepository struct {
...
// NewSQLServerLoanPaymentRepository creates a new SQL Server loan_payment repository.
func NewSQLServerLoanPaymentRepository(dbOps interfaces.DatabaseOperation, tableName string) loanpaymentpb.LoanPaymentDomainServiceServer {
	if tableName == "" {
	...
	return &SQLServerLoanPaymentRepository{
		dbOps:     dbOps,
		db:        db,
		tableName: tableName,
	}
...
func (r *SQLServerLoanPaymentRepository) CreateLoanPayment(ctx context.Context, req *loanpaymentpb.CreateLoanPaymentRequest) (*loanpaymentpb.CreateLoanPaymentResponse, error) {
	if req.Data == nil {
...
func (r *SQLServerLoanPaymentRepository) ReadLoanPayment(ctx context.Context, req *loanpaymentpb.ReadLoanPaymentRequest) (*loanpaymentpb.ReadLoanPaymentResponse, error) {
...
func (r *SQLServerLoanPaymentRepository) UpdateLoanPayment(ctx context.Context, req *loanpaymentpb.UpdateLoanPaymentRequest) (*loanpaymentpb.UpdateLoanPaymentResponse, error) {
	if req.Data == nil || req.Data.Id == "" {
		return nil, fmt.Errorf("loan_payment ID is required")
	}
...
func (r *SQLServerLoanPaymentRepository) DeleteLoanPayment(ctx context.Context, req *loanpaymentpb.DeleteLoanPaymentRequest) (*loanpaymentpb.DeleteLoanPaymentResponse, error) {
	if req.Data == nil || req.Data.Id == "" {
		return nil, fmt.Errorf("loan_payment ID is required")
	}
...
func (r *SQLServerLoanPaymentRepository) ListLoanPayments(ctx context.Context, req *loanpaymentpb.ListLoanPaymentsRequest) (*loanpaymentpb.ListLoanPaymentsResponse, error) {
...
// GetLoanPaymentListPageData retrieves loan_payments with pagination, filtering, sorting, and search.
func (r *SQLServerLoanPaymentRepository) GetLoanPaymentListPageData(
	ctx context.Context,
	...
	loanPaymentSortableCols := []string{
		"lp.date_created", "lp.payment_number", "lp.payment_date",
		"lp.principal_amount", "lp.interest_amount", "lp.total_amount",
	}

	orderByClause, err := sqlserverCore.BuildOrderBy(
		loanPaymentSortableCols,
		&commonpb.SortRequest{Fields: []*commonpb.SortField{{Field: sortColKey, Direction: sortDir}}},
		"lp.date_created DESC",
	)
	if err != nil {
		return nil, fmt.Errorf("invalid sort column for loan_payment: %w", err)
	}
...
func (r *SQLServerLoanPaymentRepository) GetLoanPaymentItemPageData(
	ctx context.Context,
	...
	exec := r.dbOps.(executorProvider).GetExecutor(ctx)
	row := exec.QueryRowContext(ctx, query, req.LoanPaymentId)
...
// NewLoanPaymentRepository creates a new SQL Server loan_payment repository (old-style constructor).
func NewLoanPaymentRepository(db *sql.DB, tableName string) loanpaymentpb.LoanPaymentDomainServiceServer {
	dbOps := sqlserverCore.NewWorkspaceAwareOperations(db)
	return NewSQLServerLoanPaymentRepository(dbOps, tableName)
}
