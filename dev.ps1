...
Write-Host "Build tags: vanilla,uuidv7,mock_db,mock_auth,mock_email,local_storage,noop"
...
air --build.cmd "go build -tags vanilla,uuidv7,mock_db,mock_auth,mock_email,local_storage,noop -o tmp/main.exe ./cmd/server" --build.exclude_dir "files,tmp,logs,node_modules,.git,assets" --build.bin "./tmp/main.exe"  --build.stop_on_error "true"