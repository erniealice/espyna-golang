...
//
// This is the real home of the contract. It is intentionally free of any
// storage-backend imports so that builds targeting non-postgres drivers
// (firestore, in-memory, etc.) can depend on this package without dragging
// in database/sql or the pq driver.
//
// The postgres implementation lives in contrib/postgres/reference/checker.go
// and satisfies this interface via a compile-time assertion. Future drivers
// (firestore, etc.) implement the same interface independently. The public
// re-export lives in github.com/erniealice/espyna-golang/ports (Checker /
// NewNoOp) so separate Go modules (contrib, blocks) can reach it.
...
func NewNoOpReferenceChecker() Checker { return NewNoOp() }
