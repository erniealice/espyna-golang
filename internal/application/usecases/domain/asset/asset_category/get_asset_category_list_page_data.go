...
func (uc *GetAssetCategoryListPageDataUseCase) Execute(ctx context.Context, req *assetcategorypb.GetAssetCategoryListPageDataRequest) (*assetcategorypb.GetAssetCategoryListPageDataResponse, error) {
	...
	if err != nil {
		translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "asset_category.errors.get_list_page_data_failed", "[ERR-DEFAULT] Failed to load asset category list")
		return nil, fmt.Errorf("%s: %w", translatedError, err)
	}
...
func (uc *GetAssetCategoryListPageDataUseCase) validateInput(ctx context.Context, req *assetcategorypb.GetAssetCategoryListPageDataRequest) error {
	...
	if req.Pagination != nil {
		if req.Pagination.Limit > 0 && (req.Pagination.Limit < 1 || req.Pagination.Limit > 100) {
			return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "asset_category.validation.invalid_pagination_limit", "[ERR-DEFAULT] Invalid pagination limit"))
		}
	}

	// Validate filter parameters
	if req.Filters != nil && len(req.Filters.Filters) > 10 {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "asset_category.validation.too_many_filters", "[ERR-DEFAULT] Too many filters"))
	}

	// Validate sort parameters
	if req.Sort != nil && len(req.Sort.Fields) > 5 {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "asset_category.validation.too_many_sort_fields", "[ERR-DEFAULT] Too many sort fields"))
	}

	// Validate search parameters
	if req.Search != nil && req.Search.Query != "" {
		if len(req.Search.Query) > 100 {
			return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "asset_category.validation.search_query_too_long", "[ERR-DEFAULT] Search query is too long"))
		}
	}
