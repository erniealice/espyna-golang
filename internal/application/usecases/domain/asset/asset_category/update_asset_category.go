...
func (uc *UpdateAssetCategoryUseCase) Execute(ctx context.Context, req *assetcategorypb.UpdateAssetCategoryRequest) (*assetcategorypb.UpdateAssetCategoryResponse, error) {
	...
	if err := authcheck.Check(ctx, uc.services.Authorizer, uc.services.Translator,
		entityAssetCategory, entityid.ActionUpdate); err != nil {
		return nil, err
	}
	...
	if err := uc.enrichAssetCategoryData(req.Data); err != nil {
		translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "asset_category.errors.enrichment_failed", "[ERR-DEFAULT] Data enrichment failed")
		return nil, fmt.Errorf("%s: %w", translatedError, err)
	}
	...
	if err != nil {
		translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "asset_category.errors.update_failed", "[ERR-DEFAULT] Asset category update failed")
		return nil, fmt.Errorf("%s: %w", translatedError, err)
	}
...
func (uc *UpdateAssetCategoryUseCase) validateInput(ctx context.Context, req *assetcategorypb.UpdateAssetCategoryRequest) error {
	...
	if req.Data.Id == "" {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "asset_category.validation.id_required", "[ERR-DEFAULT] Asset category ID is required"))
	}
	if req.Data.Name == "" {
