...
func (uc *UpdateClientAttributeUseCase) Execute(ctx context.Context, req *clientattributepb.UpdateClientAttributeRequest) (*clientattributepb.UpdateClientAttributeResponse, error) {
	...
	if err := authcheck.Check(ctx, uc.services.Authorizer, uc.services.Translator,
		entityid.ClientAttribute, entityid.ActionUpdate); err != nil {
		return nil, err
	}
	...
	if err != nil {
		translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "client_attribute.errors.update_failed", "Client attribute update failed [DEFAULT]")
		return nil, fmt.Errorf("%s: %w", translatedError, err)
	}
...
func (uc *UpdateClientAttributeUseCase) validateInput(ctx context.Context, req *clientattributepb.UpdateClientAttributeRequest) error {
	...
	if req.Data.ClientId == "" {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "client_attribute.validation.client_id_required", "Client ID is required [DEFAULT]"))
	}
	if req.Data.AttributeId == "" {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "client_attribute.validation.attribute_id_required", "Attribute ID is required [DEFAULT]"))
	}
	if req.Data.Value == "" {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "client_attribute.validation.value_required", "Value is required [DEFAULT]"))
	}
	return nil
}
...
func (uc *UpdateClientAttributeUseCase) validateEntityReferences(ctx context.Context, clientAttribute *clientattributepb.ClientAttribute) error {
	...
	if clientAttribute.ClientId != "" {
		...
		if client == nil || client.Data == nil || len(client.Data) == 0 {
			translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "client_attribute.errors.client_not_found", "Referenced client with ID '{clientId}' does not exist [DEFAULT]")
	...
	if clientAttribute.AttributeId != "" {
		...
		if attribute == nil || attribute.Data == nil || len(attribute.Data) == 0 {
			translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "client_attribute.errors.attribute_not_found", "Referenced attribute with ID '{attributeId}' does not exist [DEFAULT]")
