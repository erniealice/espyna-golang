...
func (uc *GetDelegateItemPageDataUseCase) Execute(ctx context.Context, req *delegatepb.GetDelegateItemPageDataRequest) (*delegatepb.GetDelegateItemPageDataResponse, error) {
	...
	if err != nil {
		translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "delegate.errors.get_item_page_data_failed", "[ERR-DEFAULT] Failed to load delegate details")
		return nil, fmt.Errorf("%s: %w", translatedError, err)
	}
...
func (uc *GetDelegateItemPageDataUseCase) validateInput(ctx context.Context, req *delegatepb.GetDelegateItemPageDataRequest) error {
	...
	if req.DelegateId == "" {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "delegate.validation.delegate_id_required", "[ERR-DEFAULT] Delegate ID is required"))
	}

	// Basic ID format validation
	if len(req.DelegateId) < 3 || len(req.DelegateId) > 100 {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "delegate.validation.invalid_delegate_id_format", "[ERR-DEFAULT] Invalid delegate ID format"))
	}

	// Ensure ID doesn't contain invalid characters
	if strings.ContainsAny(req.DelegateId, " \t\n\r") {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "delegate.validation.delegate_id_invalid_characters", "[ERR-DEFAULT] Delegate ID contains invalid characters"))
	}
