...
func (uc *UpdateDelegateAttributeUseCase) Execute(ctx context.Context, req *delegateattributepb.UpdateDelegateAttributeRequest) (*delegateattributepb.UpdateDelegateAttributeResponse, error) {
	...
	if err != nil {
		translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "delegate_attribute.errors.update_failed", "Delegate attribute update failed [DEFAULT]")
		return nil, fmt.Errorf("%s: %w", translatedError, err)
	}
...
func (uc *UpdateDelegateAttributeUseCase) validateInput(ctx context.Context, req *delegateattributepb.UpdateDelegateAttributeRequest) error {
	...
	if req.Data.DelegateId == "" {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "delegate_attribute.validation.delegate_id_required", "Delegate ID is required [DEFAULT]"))
	}
	if req.Data.AttributeId == "" {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "delegate_attribute.validation.attribute_id_required", "Attribute ID is required [DEFAULT]"))
	}
	if req.Data.Value == "" {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "delegate_attribute.validation.value_required", "Value is required [DEFAULT]"))
	}
	return nil
}
...
func (uc *UpdateDelegateAttributeUseCase) validateEntityReferences(ctx context.Context, delegateAttribute *delegateattributepb.DelegateAttribute) error {
	...
	if delegateAttribute.DelegateId != "" {
		...
		if delegate == nil || delegate.Data == nil || len(delegate.Data) == 0 {
			translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "delegate_attribute.errors.delegate_not_found", "Referenced delegate with ID '{delegateId}' does not exist [DEFAULT]")
	...
	if delegateAttribute.AttributeId != "" {
		...
		if attribute == nil || attribute.Data == nil || len(attribute.Data) == 0 {
			translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "delegate_attribute.errors.attribute_not_found", "Referenced attribute with ID '{attributeId}' does not exist [DEFAULT]")
