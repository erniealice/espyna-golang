...
func (uc *UpdateLocationUseCase) Execute(ctx context.Context, req *locationpb.UpdateLocationRequest) (*locationpb.UpdateLocationResponse, error) {
	...
	if err := authcheck.Check(ctx, uc.services.Authorizer, uc.services.Translator,
		entityid.Location, entityid.ActionUpdate); err != nil {
		return nil, err
	}
	...
	if err := uc.enrichLocationData(req.Data); err != nil {
		translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "location.errors.enrichment_failed", "[ERR-DEFAULT] Data enrichment failed")
		return nil, fmt.Errorf("%s: %w", translatedError, err)
	}
	...
	if err != nil {
		translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "location.errors.update_failed", "[ERR-DEFAULT] Location update failed")
		return nil, fmt.Errorf("%s: %w", translatedError, err)
	}
...
func (uc *UpdateLocationUseCase) validateInput(ctx context.Context, req *locationpb.UpdateLocationRequest) error {
	...
	if req.Data.Id == "" {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "location.validation.id_required", "[ERR-DEFAULT] Location ID is required"))
	}
	if req.Data.Name == "" {
