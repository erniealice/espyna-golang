...
func (uc *ReadRolePermissionUseCase) Execute(ctx context.Context, req *rolepermissionpb.ReadRolePermissionRequest) (*rolepermissionpb.ReadRolePermissionResponse, error) {
	...
	if err := authcheck.Check(ctx, uc.services.Authorizer, uc.services.Translator,
		ports.EntityRolePermission, ports.ActionRead); err != nil {
		return nil, err
	}
	...
	if err != nil {
		translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "role_permission.errors.read_failed", "Failed to read role permission")
		return nil, fmt.Errorf("%s: %w", translatedError, err)
	}

	// Not found error
	if len(resp.Data) == 0 || resp.Data[0].Id == "" { // Assuming resp.Data will be nil or have empty ID if not found
		translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "role_permission.errors.not_found", "Role-Permission with ID \"{rolePermissionId}\" not found")
