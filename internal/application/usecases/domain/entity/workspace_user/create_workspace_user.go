...
func (uc *CreateWorkspaceUserUseCase) Execute(ctx context.Context, req *workspaceuserpb.CreateWorkspaceUserRequest) (*workspaceuserpb.CreateWorkspaceUserResponse, error) {
	...
	if err := authcheck.Check(ctx, uc.services.Authorizer, uc.services.Translator,
		entityid.WorkspaceUser, entityid.ActionCreate); err != nil {
		return nil, err
	}

	// Business logic and enrichment
	if err := uc.enrichWorkspaceUserData(req.Data); err != nil {
		translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "workspace_user.errors.enrichment_failed", "Business logic enrichment failed [DEFAULT]")
		return nil, fmt.Errorf("%s: %w", translatedError, err)
	}

	// Business rule validation
	if err := uc.validateBusinessRules(ctx, req.Data); err != nil {
		translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "workspace_user.errors.business_rule_validation_failed", "Business rule validation failed [DEFAULT]")
		return nil, fmt.Errorf("%s: %w", translatedError, err)
	}

	// Entity reference validation
	if err := uc.validateEntityReferences(ctx, req.Data); err != nil {
		translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "workspace_user.errors.reference_validation_failed", "Entity reference validation failed [DEFAULT]")
		return nil, fmt.Errorf("%s: %w", translatedError, err)
	}
	...
	if err != nil {
		translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "workspace_user.errors.creation_failed", "Workspace-User creation failed [DEFAULT]")
		return nil, fmt.Errorf("%s: %w", translatedError, err)
	}
...
func (uc *CreateWorkspaceUserUseCase) validateInput(ctx context.Context, req *workspaceuserpb.CreateWorkspaceUserRequest) error {
	if req == nil {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "workspace_user.validation.request_required", "Request is required for workspace users [DEFAULT]"))
	}
	if req.Data == nil {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "workspace_user.validation.data_required", "Workspace-User data is required [DEFAULT]"))
	}
	if req.Data.WorkspaceId == "" {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "workspace_user.validation.workspace_id_required", "Workspace ID is required [DEFAULT]"))
	}
	if req.Data.UserId == "" {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "workspace_user.validation.user_id_required", "User ID is required [DEFAULT]"))
	}
	return nil
}
...
func (uc *CreateWorkspaceUserUseCase) enrichWorkspaceUserData(workspaceUser *workspaceuserpb.WorkspaceUser) error {
	...
	if workspaceUser.Id == "" {
		workspaceUser.Id = uc.services.IDGenerator.GenerateID()
	}
...
func (uc *CreateWorkspaceUserUseCase) validateBusinessRules(ctx context.Context, workspaceUser *workspaceuserpb.WorkspaceUser) error {
	...
	if workspaceUser.WorkspaceId == workspaceUser.UserId {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "workspace_user.validation.same_id", "Workspace ID and user ID cannot be the same [DEFAULT]"))
	}
	...
	if len(workspaceUser.WorkspaceUserRoles) > 0 {
		for _, userRole := range workspaceUser.WorkspaceUserRoles {
			if userRole == nil {
				return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "workspace_user.validation.null_role", "Workspace user roles cannot contain null entries [DEFAULT]"))
			}
			// Validate the role within the userRole object
			if userRole.Role != nil && strings.TrimSpace(userRole.Role.Name) == "" {
				return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "workspace_user.validation.empty_role", "Roles cannot contain empty strings [DEFAULT]"))
			}
		}
		...
		for _, userRole := range workspaceUser.WorkspaceUserRoles {
			if userRole != nil && userRole.Role != nil {
				...
				if roleSet[roleName] {
					return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "workspace_user.validation.duplicate_roles", "Duplicate roles are not allowed [DEFAULT]"))
				}
...
func (uc *CreateWorkspaceUserUseCase) validateEntityReferences(ctx context.Context, workspaceUser *workspaceuserpb.WorkspaceUser) error {
	...
	if workspaceUser.WorkspaceId != "" {
		...
		if err != nil {
			translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "workspace_user.errors.workspace_reference_validation_failed", "Failed to validate workspace entity reference [DEFAULT]")
			return fmt.Errorf("%s: %w", translatedError, err)
		}
		if workspace == nil || workspace.Data == nil || len(workspace.Data) == 0 {
			translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "workspace_user.errors.workspace_not_found", "Referenced workspace with ID '{workspaceId}' does not exist [DEFAULT]")
			translatedError = strings.ReplaceAll(translatedError, "{workspaceId}", workspaceUser.WorkspaceId)
			return errors.New(translatedError)
		}
		if !workspace.Data[0].Active {
			translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "workspace_user.errors.workspace_not_active", "Referenced workspace with ID '{workspaceId}' is not active [DEFAULT]")
			translatedError = strings.ReplaceAll(translatedError, "{workspaceId}", workspaceUser.WorkspaceId)
			return errors.New(translatedError)
		}
	}
	...
	if workspaceUser.UserId != "" {
		...
		if err != nil {
			translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "workspace_user.errors.user_reference_validation_failed", "Failed to validate user entity reference [DEFAULT]")
			return fmt.Errorf("%s: %w", translatedError, err)
		}
		if user == nil || user.Data == nil || len(user.Data) == 0 {
			translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "workspace_user.errors.user_not_found", "Referenced user with ID '{userId}' does not exist [DEFAULT]")
			translatedError = strings.ReplaceAll(translatedError, "{userId}", workspaceUser.UserId)
			return errors.New(translatedError)
		}
		if !user.Data[0].Active {
			translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "workspace_user.errors.user_not_active", "Referenced user with ID '{userId}' is not active [DEFAULT]")
			translatedError = strings.ReplaceAll(translatedError, "{userId}", workspaceUser.UserId)
			return errors.New(translatedError)
		}
	}
