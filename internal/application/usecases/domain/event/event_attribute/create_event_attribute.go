...
func (uc *CreateEventAttributeUseCase) Execute(ctx context.Context, req *eventattributepb.CreateEventAttributeRequest) (*eventattributepb.CreateEventAttributeResponse, error) {
	...
	if err := authcheck.Check(ctx, uc.services.Authorizer, uc.services.Translator,
		entityid.EventAttribute, entityid.ActionCreate); err != nil {
		return nil, err
	}
...
func (uc *CreateEventAttributeUseCase) executeWithTransaction(ctx context.Context, req *eventattributepb.CreateEventAttributeRequest) (*eventattributepb.CreateEventAttributeResponse, error) {
	...
	err := uc.services.Transactor.ExecuteInTransaction(ctx, func(txCtx context.Context) error {
		...
		if err != nil {
			return fmt.Errorf(contextutil.GetTranslatedMessageWithContext(txCtx, uc.services.Translator, "event_attribute.errors.creation_failed", "Event attribute creation failed [DEFAULT]"), err)
		}
		result = res
		return nil
	})
...
func (uc *CreateEventAttributeUseCase) executeCore(ctx context.Context, req *eventattributepb.CreateEventAttributeRequest) (*eventattributepb.CreateEventAttributeResponse, error) {
	...
	if err := uc.validateInput(ctx, req); err != nil {
		return nil, fmt.Errorf(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "event_attribute.errors.validation_failed", "Input validation failed [DEFAULT]"), err)
	}

	// Business logic and enrichment
	if err := uc.enrichEventAttributeData(req.Data); err != nil {
		return nil, fmt.Errorf(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "event_attribute.errors.enrichment_failed", "Business logic enrichment failed [DEFAULT]"), err)
	}

	// Entity reference validation
	if err := uc.validateEntityReferences(ctx, req.Data); err != nil {
		return nil, fmt.Errorf(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "event_attribute.errors.entity_reference_validation_failed", "Entity reference validation failed [DEFAULT]"), err)
	}

	// Business rule validation
	if err := uc.validateBusinessRules(ctx, req.Data); err != nil {
		return nil, fmt.Errorf(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "event_attribute.errors.business_rule_validation_failed", "Business rule validation failed [DEFAULT]"), err)
	}

	// Call repository
	resp, err := uc.repositories.EventAttribute.CreateEventAttribute(ctx, req)
	if err != nil {
		return nil, fmt.Errorf(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "event_attribute.errors.creation_failed", "Event attribute creation failed [DEFAULT]"), err)
	}
	return resp, nil
}
...
func (uc *CreateEventAttributeUseCase) enrichEventAttributeData(eventAttribute *eventattributepb.EventAttribute) error {
	...
	if eventAttribute.Id == "" {
		if uc.services.IDGenerator != nil {
			eventAttribute.Id = uc.services.IDGenerator.GenerateID()
		} else {
...
func (uc *CreateEventAttributeUseCase) validateEntityReferences(ctx context.Context, eventAttribute *eventattributepb.EventAttribute) error {
	...
	if eventAttribute.EventId != "" {
		...
		if err != nil {
			return fmt.Errorf(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "event_attribute.errors.event_reference_validation_failed", "Failed to validate event entity reference [DEFAULT]"), err)
		}
		if event == nil || event.Data == nil || len(event.Data) == 0 {
			return fmt.Errorf(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "event_attribute.errors.event_not_found", "Referenced event with ID '%s' does not exist [DEFAULT]"), eventAttribute.EventId)
		}
		if !event.Data[0].Active {
			return fmt.Errorf(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "event_attribute.errors.event_not_active", "Referenced event with ID '%s' is not active [DEFAULT]"), eventAttribute.EventId)
		}
	}
	...
	if eventAttribute.AttributeId != "" {
		...
		if err != nil {
			return fmt.Errorf(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "event_attribute.errors.attribute_reference_validation_failed", "Failed to validate attribute entity reference [DEFAULT]"), err)
		}
		if attribute == nil || attribute.Data == nil || len(attribute.Data) == 0 {
			return fmt.Errorf(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "event_attribute.errors.attribute_not_found", "Referenced attribute with ID '%s' does not exist [DEFAULT]"), eventAttribute.AttributeId)
		}
		if !attribute.Data[0].Active {
			return fmt.Errorf(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "event_attribute.errors.attribute_not_active", "Referenced attribute with ID '%s' is not active [DEFAULT]"), eventAttribute.AttributeId)
		}
	}
