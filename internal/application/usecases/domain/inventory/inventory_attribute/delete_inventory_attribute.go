...
func (uc *DeleteInventoryAttributeUseCase) executeWithTransaction(ctx context.Context, req *inventoryattributepb.DeleteInventoryAttributeRequest) (*inventoryattributepb.DeleteInventoryAttributeResponse, error) {
	...
	if err != nil {
		translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "inventory_attribute.errors.transaction_failed", "Transaction execution failed [DEFAULT]")
...
func (uc *DeleteInventoryAttributeUseCase) executeCore(ctx context.Context, req *inventoryattributepb.DeleteInventoryAttributeRequest) (*inventoryattributepb.DeleteInventoryAttributeResponse, error) {
	...
	permission := entityid.EntityPermission(entityid.InventoryAttribute, entityid.ActionDelete)
	...
	if err != nil {
		translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "inventory_attribute.errors.deletion_failed", "Inventory attribute deletion failed [DEFAULT]")
		return nil, fmt.Errorf("%s: %w", translatedError, err)
	}
	return resp, nil
}
