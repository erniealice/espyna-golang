...
func (uc *ListInventoryAttributesUseCase) Execute(ctx context.Context, req *inventoryattributepb.ListInventoryAttributesRequest) (*inventoryattributepb.ListInventoryAttributesResponse, error) {
	if err := authcheck.Check(ctx, uc.services.Authorizer, uc.services.Translator,
		entityid.InventoryAttribute, entityid.ActionList); err != nil {
		return nil, err
	}
	...
	permission := entityid.EntityPermission(entityid.InventoryAttribute, entityid.ActionList)
	...
	if err != nil {
		translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "inventory_attribute.errors.list_failed", "Failed to retrieve inventory attributes [DEFAULT]")
		return nil, fmt.Errorf("%s: %w", translatedError, err)
	}
	return resp, nil
}
