...
func (uc *UpdateInventoryAttributeUseCase) executeWithTransaction(ctx context.Context, req *inventoryattributepb.UpdateInventoryAttributeRequest) (*inventoryattributepb.UpdateInventoryAttributeResponse, error) {
	...
	err := uc.services.Transactor.ExecuteInTransaction(ctx, func(txCtx context.Context) error {
		...
		if err != nil {
			translatedError := contextutil.GetTranslatedMessageWithContext(txCtx, uc.services.Translator, "inventory_attribute.errors.update_failed", "Inventory attribute update failed [DEFAULT]")
...
func (uc *UpdateInventoryAttributeUseCase) executeCore(ctx context.Context, req *inventoryattributepb.UpdateInventoryAttributeRequest) (*inventoryattributepb.UpdateInventoryAttributeResponse, error) {
	...
	permission := entityid.EntityPermission(entityid.InventoryAttribute, entityid.ActionUpdate)
	...
	if err != nil || existingResp == nil || len(existingResp.Data) == 0 {
		translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "inventory_attribute.errors.not_found", "Inventory attribute not found for update [DEFAULT]")
		return nil, errors.New(translatedError)
	}
	...
	if err != nil {
		translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "inventory_attribute.errors.update_failed", "Inventory attribute update failed [DEFAULT]")
		return nil, fmt.Errorf("%s: %w", translatedError, err)
	}
	return resp, nil
}
