...
func (uc *CreateInventorySerialUseCase) Execute(ctx context.Context, req *inventoryserialpb.CreateInventorySerialRequest) (*inventoryserialpb.CreateInventorySerialResponse, error) {
	if err := authcheck.Check(ctx, uc.services.Authorizer, uc.services.Translator,
		entityid.InventorySerial, entityid.ActionCreate); err != nil {
		return nil, err
	}
...
func (uc *CreateInventorySerialUseCase) executeCore(ctx context.Context, req *inventoryserialpb.CreateInventorySerialRequest) (*inventoryserialpb.CreateInventorySerialResponse, error) {
	...
	permission := entityid.EntityPermission(entityid.InventorySerial, entityid.ActionCreate)
	...
	if err := uc.enrichData(req.Data); err != nil {
		translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "inventory_serial.errors.enrichment_failed", "Business logic enrichment failed [DEFAULT]")
		return nil, fmt.Errorf("%s: %w", translatedError, err)
	}

	if err := uc.validateBusinessRules(ctx, req.Data); err != nil {
		translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "inventory_serial.errors.business_rule_validation_failed", "Business rule validation failed [DEFAULT]")
		return nil, fmt.Errorf("%s: %w", translatedError, err)
	}
...
func (uc *CreateInventorySerialUseCase) validateInput(ctx context.Context, req *inventoryserialpb.CreateInventorySerialRequest) error {
	...
	if req.Data.SerialNumber == "" {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "inventory_serial.validation.serial_number_required", "Serial number is required [DEFAULT]"))
	}
	if req.Data.InventoryItemId == "" {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "inventory_serial.validation.inventory_item_id_required", "Inventory item ID is required [DEFAULT]"))
	}
	if req.Data.Status == "" {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "inventory_serial.validation.status_required", "Status is required [DEFAULT]"))
	}
	return nil
}
...
func (uc *CreateInventorySerialUseCase) enrichData(serial *inventoryserialpb.InventorySerial) error {
	...
	if serial.Id == "" {
		serial.Id = uc.services.IDGenerator.GenerateID()
	}
...
func (uc *CreateInventorySerialUseCase) validateBusinessRules(ctx context.Context, serial *inventoryserialpb.InventorySerial) error {
	...
	if !validStatuses[serial.Status] {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "inventory_serial.validation.invalid_status", "Status must be available, reserved, sold, damaged, or returned [DEFAULT]"))
	}
	return nil
}
