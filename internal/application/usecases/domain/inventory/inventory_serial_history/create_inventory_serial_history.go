...
func (uc *CreateInventorySerialHistoryUseCase) Execute(ctx context.Context, req *serialhistorypb.CreateInventorySerialHistoryRequest) (*serialhistorypb.CreateInventorySerialHistoryResponse, error) {
	if err := authcheck.Check(ctx, uc.services.Authorizer, uc.services.Translator,
		entityid.InventorySerialHistory, entityid.ActionCreate); err != nil {
		return nil, err
	}
...
func (uc *CreateInventorySerialHistoryUseCase) executeCore(ctx context.Context, req *serialhistorypb.CreateInventorySerialHistoryRequest) (*serialhistorypb.CreateInventorySerialHistoryResponse, error) {
	...
	permission := entityid.EntityPermission(entityid.InventorySerialHistory, entityid.ActionCreate)
	...
	if err := uc.enrichData(req.Data); err != nil {
		translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "inventory_serial_history.errors.enrichment_failed", "Business logic enrichment failed [DEFAULT]")
		return nil, fmt.Errorf("%s: %w", translatedError, err)
	}

	if err := uc.validateBusinessRules(ctx, req.Data); err != nil {
		translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "inventory_serial_history.errors.business_rule_validation_failed", "Business rule validation failed [DEFAULT]")
		return nil, fmt.Errorf("%s: %w", translatedError, err)
	}
...
func (uc *CreateInventorySerialHistoryUseCase) validateInput(ctx context.Context, req *serialhistorypb.CreateInventorySerialHistoryRequest) error {
	...
	if req.Data.InventorySerialId == "" {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "inventory_serial_history.validation.inventory_serial_id_required", "Inventory serial ID is required [DEFAULT]"))
	}
	if req.Data.InventoryItemId == "" {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "inventory_serial_history.validation.inventory_item_id_required", "Inventory item ID is required [DEFAULT]"))
	}
	if req.Data.FromStatus == "" {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "inventory_serial_history.validation.from_status_required", "From status is required [DEFAULT]"))
	}
	if req.Data.ToStatus == "" {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "inventory_serial_history.validation.to_status_required", "To status is required [DEFAULT]"))
	}
	if req.Data.ReferenceType == "" {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "inventory_serial_history.validation.reference_type_required", "Reference type is required [DEFAULT]"))
	}
	return nil
}
...
func (uc *CreateInventorySerialHistoryUseCase) validateBusinessRules(ctx context.Context, history *serialhistorypb.InventorySerialHistory) error {
	...
	if history.FromStatus == history.ToStatus {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "inventory_serial_history.validation.status_must_change", "From status and to status must be different [DEFAULT]"))
	}
	...
	if !validReferenceTypes[history.ReferenceType] {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "inventory_serial_history.validation.invalid_reference_type", "Reference type must be sale, manual, return, damage, repair, or transfer [DEFAULT]"))
	}
