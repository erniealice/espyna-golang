...
func (uc *CreateInventoryTransactionUseCase) Execute(ctx context.Context, req *inventorytransactionpb.CreateInventoryTransactionRequest) (*inventorytransactionpb.CreateInventoryTransactionResponse, error) {
	if err := authcheck.Check(ctx, uc.services.Authorizer, uc.services.Translator,
		entityid.InventoryTransaction, entityid.ActionCreate); err != nil {
		return nil, err
	}
...
func (uc *CreateInventoryTransactionUseCase) executeCore(ctx context.Context, req *inventorytransactionpb.CreateInventoryTransactionRequest) (*inventorytransactionpb.CreateInventoryTransactionResponse, error) {
	...
	permission := entityid.EntityPermission(entityid.InventoryTransaction, entityid.ActionCreate)
	...
	if err := uc.enrichData(req.Data); err != nil {
		translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "inventory_transaction.errors.enrichment_failed", "Business logic enrichment failed [DEFAULT]")
		return nil, fmt.Errorf("%s: %w", translatedError, err)
	}

	if err := uc.validateBusinessRules(ctx, req.Data); err != nil {
		translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "inventory_transaction.errors.business_rule_validation_failed", "Business rule validation failed [DEFAULT]")
		return nil, fmt.Errorf("%s: %w", translatedError, err)
	}
...
func (uc *CreateInventoryTransactionUseCase) validateInput(ctx context.Context, req *inventorytransactionpb.CreateInventoryTransactionRequest) error {
	...
	if req.Data.InventoryItemId == "" {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "inventory_transaction.validation.inventory_item_id_required", "Inventory item ID is required [DEFAULT]"))
	}
	if req.Data.TransactionType == "" {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "inventory_transaction.validation.transaction_type_required", "Transaction type is required [DEFAULT]"))
	}
	if req.Data.Quantity == 0 {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "inventory_transaction.validation.quantity_non_zero", "Quantity must not be zero [DEFAULT]"))
	}
	return nil
}
...
func (uc *CreateInventoryTransactionUseCase) enrichData(transaction *inventorytransactionpb.InventoryTransaction) error {
	...
	if transaction.Id == "" {
		transaction.Id = uc.services.IDGenerator.GenerateID()
	}
...
func (uc *CreateInventoryTransactionUseCase) validateBusinessRules(ctx context.Context, transaction *inventorytransactionpb.InventoryTransaction) error {
	...
	if !validTypes[transaction.TransactionType] {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "inventory_transaction.validation.invalid_transaction_type", "Transaction type must be receipt, issue, transfer, adjustment, or return [DEFAULT]"))
	}
	return nil
}
