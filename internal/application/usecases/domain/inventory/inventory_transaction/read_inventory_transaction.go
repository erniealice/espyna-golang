...
func (uc *ReadInventoryTransactionUseCase) Execute(ctx context.Context, req *inventorytransactionpb.ReadInventoryTransactionRequest) (*inventorytransactionpb.ReadInventoryTransactionResponse, error) {
	if err := authcheck.Check(ctx, uc.services.Authorizer, uc.services.Translator,
		entityid.InventoryTransaction, entityid.ActionRead); err != nil {
		return nil, err
	}
	...
	permission := entityid.EntityPermission(entityid.InventoryTransaction, entityid.ActionRead)
	...
	if err != nil {
		...
		if strings.Contains(errorMsg, "not found") {
			translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "inventory_transaction.errors.not_found", "Inventory transaction not found [DEFAULT]")
			return nil, errors.New(translatedError)
		}
		translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "inventory_transaction.errors.read_failed", "Failed to retrieve inventory transaction [DEFAULT]")
		return nil, fmt.Errorf("%s: %w", translatedError, err)
	}
	return resp, nil
}
