...
func (uc *UpdateJournalEntryUseCase) Execute(ctx context.Context, req *journalentrypb.UpdateJournalEntryRequest) (*journalentrypb.UpdateJournalEntryResponse, error) {
	...
	if err := uc.enrichJournalEntryData(req.Data); err != nil {
		translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "journal_entry.errors.enrichment_failed", "[ERR-DEFAULT] Data enrichment failed")
		return nil, fmt.Errorf("%s: %w", translatedError, err)
	}
	...
	if err != nil {
		translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "journal_entry.errors.update_failed", "[ERR-DEFAULT] Journal entry update failed")
		return nil, fmt.Errorf("%s: %w", translatedError, err)
	}
...
func (uc *UpdateJournalEntryUseCase) validateBusinessRules(ctx context.Context, entry *journalentrypb.JournalEntry) error {
	...
	if entry.Status != journalentrypb.JournalEntryStatus_JOURNAL_ENTRY_STATUS_DRAFT &&
		...
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "journal_entry.validation.only_draft_editable", "[ERR-DEFAULT] Only DRAFT journal entries can be edited"))
	}
