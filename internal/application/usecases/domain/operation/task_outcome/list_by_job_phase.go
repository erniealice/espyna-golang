...
func (uc *ListByJobPhaseUseCase) executeWithTransaction(ctx context.Context, req *pb.ListTaskOutcomesByJobPhaseRequest) (*pb.ListTaskOutcomesByJobPhaseResponse, error) {
	...
	err := uc.services.Transactor.ExecuteInTransaction(ctx, func(txCtx context.Context) error {
		...
		if err != nil {
			return fmt.Errorf(contextutil.GetTranslatedMessageWithContext(txCtx, uc.services.Translator, "task_outcome.errors.list_by_job_phase_failed", "task outcome listing by job phase failed: %w"), err)
		}
		result = res
		return nil
	})
...
func (uc *ListByJobPhaseUseCase) executeCore(ctx context.Context, req *pb.ListTaskOutcomesByJobPhaseRequest) (*pb.ListTaskOutcomesByJobPhaseResponse, error) {
	...
	if err != nil {
		return nil, fmt.Errorf(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "task_outcome.errors.list_by_job_phase_failed", "failed to list task outcomes by job phase: %w"), err)
	}
	return resp, nil
}
...
func (uc *ListByJobPhaseUseCase) validateInput(ctx context.Context, req *pb.ListTaskOutcomesByJobPhaseRequest) error {
	...
	if req.JobPhaseId == "" {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "task_outcome.validation.job_phase_id_required", "job phase ID is required"))
	}
