...
func (uc *ListByTemplateTaskUseCase) executeWithTransaction(ctx context.Context, req *pb.ListTemplateTaskCriteriasByTemplateTaskRequest) (*pb.ListTemplateTaskCriteriasByTemplateTaskResponse, error) {
	...
	err := uc.services.Transactor.ExecuteInTransaction(ctx, func(txCtx context.Context) error {
		...
		if err != nil {
			return fmt.Errorf(contextutil.GetTranslatedMessageWithContext(txCtx, uc.services.Translator, "template_task_criteria.errors.list_by_template_task_failed", "template task criteria listing by template task failed: %w"), err)
		}
		result = res
		return nil
	})
...
func (uc *ListByTemplateTaskUseCase) validateInput(ctx context.Context, req *pb.ListTemplateTaskCriteriasByTemplateTaskRequest) error {
	...
	if req.JobTemplateTaskId == "" {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "template_task_criteria.validation.task_id_required", "job template task ID is required"))
	}
