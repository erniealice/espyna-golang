...
func (uc *CreateCollectionAttributeUseCase) executeWithTransaction(ctx context.Context, req *collectionattributepb.CreateCollectionAttributeRequest) (*collectionattributepb.CreateCollectionAttributeResponse, error) {
	...
	err := uc.services.Transactor.ExecuteInTransaction(ctx, func(txCtx context.Context) error {
		...
		if err != nil {
			return fmt.Errorf(contextutil.GetTranslatedMessageWithContext(txCtx, uc.services.Translator, "collection_attribute.errors.creation_failed", "Collection attribute creation failed [DEFAULT]"), err)
		}
		result = res
		return nil
	})
...
func (uc *CreateCollectionAttributeUseCase) executeCore(ctx context.Context, req *collectionattributepb.CreateCollectionAttributeRequest) (*collectionattributepb.CreateCollectionAttributeResponse, error) {
	...
	if err := uc.validateInput(ctx, req); err != nil {
		return nil, fmt.Errorf(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "collection_attribute.errors.validation_failed", "Input validation failed [DEFAULT]"), err)
	}

	// Business logic and enrichment
	if err := uc.enrichCollectionAttributeData(req.Data); err != nil {
		return nil, fmt.Errorf(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "collection_attribute.errors.enrichment_failed", "Business logic enrichment failed [DEFAULT]"), err)
	}

	// Entity reference validation
	if err := uc.validateEntityReferences(ctx, req.Data); err != nil {
		return nil, fmt.Errorf(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "collection_attribute.errors.entity_reference_validation_failed", "Entity reference validation failed [DEFAULT]"), err)
	}

	// Business rule validation
	if err := uc.validateBusinessRules(ctx, req.Data); err != nil {
		return nil, fmt.Errorf(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "collection_attribute.errors.business_rule_validation_failed", "Business rule validation failed [DEFAULT]"), err)
	}

	// Call repository
	resp, err := uc.repositories.CollectionAttribute.CreateCollectionAttribute(ctx, req)
	if err != nil {
		return nil, fmt.Errorf(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "collection_attribute.errors.creation_failed", "Collection attribute creation failed [DEFAULT]"), err)
	}
	return resp, nil
}
...
func (uc *CreateCollectionAttributeUseCase) validateInput(ctx context.Context, req *collectionattributepb.CreateCollectionAttributeRequest) error {
	...
	if req.Data == nil {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "collection_attribute.validation.data_required", "Collection attribute data is required [DEFAULT]"))
	}
	if req.Data.CollectionId == "" {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "collection_attribute.validation.collection_id_required", "Collection ID is required [DEFAULT]"))
	}
	if req.Data.AttributeId == "" {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "collection_attribute.validation.attribute_id_required", "Attribute ID is required [DEFAULT]"))
	}
	if req.Data.Value == "" {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "collection_attribute.validation.value_required", "Attribute value is required [DEFAULT]"))
	}
	return nil
}
...
func (uc *CreateCollectionAttributeUseCase) enrichCollectionAttributeData(collectionAttribute *collectionattributepb.CollectionAttribute) error {
	...
	if collectionAttribute.Id == "" {
		if uc.services.IDGenerator != nil {
			collectionAttribute.Id = uc.services.IDGenerator.GenerateID()
		} else {
...
func (uc *CreateCollectionAttributeUseCase) validateBusinessRules(ctx context.Context, collectionAttribute *collectionattributepb.CollectionAttribute) error {
	...
	if len(collectionAttribute.CollectionId) < 5 {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "collection_attribute.validation.collection_id_min_length", "Collection ID must be at least 5 characters long [DEFAULT]"))
	}

	// Validate attribute ID format
	if len(collectionAttribute.AttributeId) < 2 {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "collection_attribute.validation.attribute_id_min_length", "Attribute ID must be at least 2 characters long [DEFAULT]"))
	}

	// Validate attribute value length
	value := strings.TrimSpace(collectionAttribute.Value)
	if len(value) < 1 {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "collection_attribute.validation.value_not_empty", "Attribute value must not be empty [DEFAULT]"))
	}

	if len(value) > 500 {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "collection_attribute.validation.value_max_length", "Attribute value cannot exceed 500 characters [DEFAULT]"))
	}
	...
	if collectionAttribute.CollectionId == "" {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "collection_attribute.validation.collection_association_required", "Collection attribute must be associated with a collection [DEFAULT]"))
	}
...
func (uc *CreateCollectionAttributeUseCase) validateEntityReferences(ctx context.Context, collectionAttribute *collectionattributepb.CollectionAttribute) error {
	...
	if collectionAttribute.CollectionId != "" {
		...
		if err != nil {
			return fmt.Errorf(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "collection_attribute.errors.collection_reference_validation_failed", "Failed to validate collection entity reference [DEFAULT]"), err)
		}
		if collection == nil || collection.Data == nil || len(collection.Data) == 0 {
			return fmt.Errorf(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "collection_attribute.errors.collection_not_found", "Referenced collection with ID '%s' does not exist [DEFAULT]"), collectionAttribute.CollectionId)
		}
		if !collection.Data[0].Active {
			return fmt.Errorf(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "collection_attribute.errors.collection_not_active", "Referenced collection with ID '%s' is not active [DEFAULT]"), collectionAttribute.CollectionId)
		}
	}
	...
	if collectionAttribute.AttributeId != "" {
		...
		if err != nil {
			return fmt.Errorf(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "collection_attribute.errors.attribute_reference_validation_failed", "Failed to validate attribute entity reference [DEFAULT]"), err)
		}
		if attribute == nil || attribute.Data == nil || len(attribute.Data) == 0 {
			return fmt.Errorf(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "collection_attribute.errors.attribute_not_found", "Referenced attribute with ID '%s' does not exist [DEFAULT]"), collectionAttribute.AttributeId)
		}
		if !attribute.Data[0].Active {
			return fmt.Errorf(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "collection_attribute.errors.attribute_not_active", "Referenced attribute with ID '%s' is not active [DEFAULT]"), collectionAttribute.AttributeId)
		}
	}
