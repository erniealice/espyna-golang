...
func (uc *CreatePriceListUseCase) Execute(ctx context.Context, req *pricelistpb.CreatePriceListRequest) (*pricelistpb.CreatePriceListResponse, error) {
	...
	if err := uc.enrichPriceListData(req.Data); err != nil {
		translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "price_list.errors.enrichment_failed", "Business logic enrichment failed [DEFAULT]")
		return nil, fmt.Errorf("%s: %w", translatedError, err)
	}
...
func (uc *CreatePriceListUseCase) executeWithTransaction(ctx context.Context, req *pricelistpb.CreatePriceListRequest) (*pricelistpb.CreatePriceListResponse, error) {
	...
	err := uc.services.Transactor.ExecuteInTransaction(ctx, func(txCtx context.Context) error {
		...
		if err := uc.validateBusinessRules(txCtx, req.Data); err != nil {
			translatedError := contextutil.GetTranslatedMessageWithContext(txCtx, uc.services.Translator, "price_list.errors.business_rule_validation_failed", "Business rule validation failed [DEFAULT]")
		...
		if err != nil {
			translatedError := contextutil.GetTranslatedMessageWithContext(txCtx, uc.services.Translator, "price_list.errors.creation_failed", "Failed to create price list [DEFAULT]")
			return fmt.Errorf("%s: %w", translatedError, err)
		}
...
func (uc *CreatePriceListUseCase) executeWithoutTransaction(ctx context.Context, req *pricelistpb.CreatePriceListRequest) (*pricelistpb.CreatePriceListResponse, error) {
	...
	if err != nil {
		translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "price_list.errors.creation_failed", "Failed to create price list [DEFAULT]")
		return nil, fmt.Errorf("%s: %w", translatedError, err)
	}
	return resp, nil
}
...
func (uc *CreatePriceListUseCase) validateInput(ctx context.Context, req *pricelistpb.CreatePriceListRequest) error {
	if req == nil {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "price_list.validation.request_required", "Request is required for price list [DEFAULT]"))
	}
	if req.Data == nil {
	...
	if req.Data.Name == "" {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "price_list.validation.name_required", "Price list name is required [DEFAULT]"))
	}
	return nil
}
...
func (uc *CreatePriceListUseCase) enrichPriceListData(priceList *pricelistpb.PriceList) error {
	...
	if priceList.Id == "" {
		priceList.Id = uc.services.IDGenerator.GenerateID()
	}
...
func (uc *CreatePriceListUseCase) validateBusinessRules(ctx context.Context, priceList *pricelistpb.PriceList) error {
	...
	if len(priceList.Name) < 3 {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "price_list.validation.name_min_length", "Price list name must be at least 3 characters long [DEFAULT]"))
	}

	if len(priceList.Name) > 100 {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "price_list.validation.name_max_length", "Price list name cannot exceed 100 characters [DEFAULT]"))
	}
