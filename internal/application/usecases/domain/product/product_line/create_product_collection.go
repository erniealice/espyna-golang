...
func (uc *CreateProductLineUseCase) executeWithTransaction(ctx context.Context, req *productlinepb.CreateProductLineRequest) (*productlinepb.CreateProductLineResponse, error) {
	...
	err := uc.services.Transactor.ExecuteInTransaction(ctx, func(txCtx context.Context) error {
		...
		if err != nil {
			translatedError := contextutil.GetTranslatedMessageWithContext(txCtx, uc.services.Translator, "product_line.errors.creation_failed", "Product line creation failed [DEFAULT]")
...
func (uc *CreateProductLineUseCase) executeCore(ctx context.Context, req *productlinepb.CreateProductLineRequest) (*productlinepb.CreateProductLineResponse, error) {
	...
	if err != nil {
		translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "product_line.errors.creation_failed", "Product line creation failed [DEFAULT]")
		return nil, fmt.Errorf("%s: %w", translatedError, err)
	}
	return resp, nil
}
...
func (uc *CreateProductLineUseCase) enrichProductLineData(productLine *productlinepb.ProductLine) error {
	...
	if productLine.Id == "" {
		productLine.Id = uc.services.IDGenerator.GenerateID()
	}
