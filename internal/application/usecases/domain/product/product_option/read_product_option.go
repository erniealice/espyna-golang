...
func (uc *ReadProductOptionUseCase) Execute(ctx context.Context, req *productoptionpb.ReadProductOptionRequest) (*productoptionpb.ReadProductOptionResponse, error) {
	...
	if err := authcheck.Check(ctx, uc.services.Authorizer, uc.services.Translator,
		entityid.ProductOption, entityid.ActionRead); err != nil {
		return nil, err
	}
	...
	permission := entityid.EntityPermission(entityid.ProductOption, entityid.ActionRead)
	...
	if err != nil {
		...
		if strings.Contains(errorMsg, "not found") {
			translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "product_option.errors.not_found", "Product option not found [DEFAULT]")
			return nil, errors.New(translatedError)
		}

		// Handle other repository errors
		translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "product_option.errors.read_failed", "Failed to retrieve product option [DEFAULT]")
		return nil, fmt.Errorf("%s: %w", translatedError, err)
	}
