...
func (uc *UpdateProductOptionUseCase) Execute(ctx context.Context, req *productoptionpb.UpdateProductOptionRequest) (*productoptionpb.UpdateProductOptionResponse, error) {
	...
	if err := authcheck.Check(ctx, uc.services.Authorizer, uc.services.Translator,
		entityid.ProductOption, entityid.ActionUpdate); err != nil {
		return nil, err
	}
...
func (uc *UpdateProductOptionUseCase) executeWithTransaction(ctx context.Context, req *productoptionpb.UpdateProductOptionRequest) (*productoptionpb.UpdateProductOptionResponse, error) {
	...
	err := uc.services.Transactor.ExecuteInTransaction(ctx, func(txCtx context.Context) error {
		...
		if err != nil {
			translatedError := contextutil.GetTranslatedMessageWithContext(txCtx, uc.services.Translator, "product_option.errors.update_failed", "Product option update failed [DEFAULT]")
...
func (uc *UpdateProductOptionUseCase) executeCore(ctx context.Context, req *productoptionpb.UpdateProductOptionRequest) (*productoptionpb.UpdateProductOptionResponse, error) {
	...
	permission := entityid.EntityPermission(entityid.ProductOption, entityid.ActionUpdate)
	...
	if err != nil {
		translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "product_option.errors.update_failed", "Product option update failed [DEFAULT]")
		return nil, fmt.Errorf("%s: %w", translatedError, err)
	}
	return resp, nil
}
