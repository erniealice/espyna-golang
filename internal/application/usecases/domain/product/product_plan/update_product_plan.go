...
func (uc *UpdateProductPlanUseCase) Execute(ctx context.Context, req *productplanpb.UpdateProductPlanRequest) (*productplanpb.UpdateProductPlanResponse, error) {
	...
	if err := authcheck.Check(ctx, uc.services.Authorizer, uc.services.Translator,
		entityid.ProductPlan, entityid.ActionUpdate); err != nil {
		return nil, err
	}
	...
	permission := entityid.EntityPermission(entityid.ProductPlan, entityid.ActionUpdate)
...
func (uc *UpdateProductPlanUseCase) executeWithTransaction(ctx context.Context, req *productplanpb.UpdateProductPlanRequest) (*productplanpb.UpdateProductPlanResponse, error) {
	...
	err := uc.services.Transactor.ExecuteInTransaction(ctx, func(txCtx context.Context) error {
		...
		if err != nil {
			msg := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "product_plan.errors.update_failed", "Product plan update failed [DEFAULT]")
...
func (uc *UpdateProductPlanUseCase) executeCore(ctx context.Context, req *productplanpb.UpdateProductPlanRequest) (*productplanpb.UpdateProductPlanResponse, error) {
	...
	if err := uc.validateEntityReferencesWithTranslation(ctx, req.Data); err != nil {
		translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "product_plan.errors.entity_reference_validation_failed", "Entity reference validation failed [DEFAULT]")
		return nil, fmt.Errorf("%s: %w", translatedError, err)
	}

	// Business rule validation with translation support
	if err := uc.validateBusinessRulesWithTranslation(ctx, req.Data); err != nil {
		translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "product_plan.errors.business_rule_validation_failed", "Business rule validation failed [DEFAULT]")
		return nil, fmt.Errorf("%s: %w", translatedError, err)
	}
	...
	if err != nil {
		...
		translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "product_plan.errors.update_failed", "Product update failed [DEFAULT]")
		return nil, fmt.Errorf("%s: %w", translatedError, err)
	}
	return resp, nil
}
...
func (uc *UpdateProductPlanUseCase) validateEntityReferencesWithTranslation(ctx context.Context, productPlan *productplanpb.ProductPlan) error {
	...
	if productPlan.ProductId != "" {
		...
		if err != nil {
			msg := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "product_plan.errors.product_validation_failed", "Referenced entity validation failed [DEFAULT]")
			return fmt.Errorf("%s: %w", msg, err)
		}
		if product == nil || product.Data == nil || len(product.Data) == 0 {
			msg := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "product_plan.errors.product_not_available", "Referenced product is not available [DEFAULT]")
