...
func (uc *ReadProductVariantUseCase) Execute(ctx context.Context, req *productvariantpb.ReadProductVariantRequest) (*productvariantpb.ReadProductVariantResponse, error) {
	...
	if err := authcheck.Check(ctx, uc.services.Authorizer, uc.services.Translator,
		entityid.ProductVariant, entityid.ActionRead); err != nil {
		return nil, err
	}
	...
	permission := entityid.EntityPermission(entityid.ProductVariant, entityid.ActionRead)
	...
	if err != nil {
		...
		if strings.Contains(errorMsg, "not found") {
			translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "product_variant.errors.not_found", "Product variant not found [DEFAULT]")
			return nil, errors.New(translatedError)
		}

		// Handle other repository errors
		translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "product_variant.errors.read_failed", "Failed to retrieve product variant [DEFAULT]")
		return nil, fmt.Errorf("%s: %w", translatedError, err)
	}
