...
func (uc *DeleteProductVariantImageUseCase) Execute(ctx context.Context, req *productvariantimagepb.DeleteProductVariantImageRequest) (*productvariantimagepb.DeleteProductVariantImageResponse, error) {
	...
	if err := authcheck.Check(ctx, uc.services.Authorizer, uc.services.Translator,
		entityid.ProductVariantImage, entityid.ActionDelete); err != nil {
		return nil, err
	}
...
func (uc *DeleteProductVariantImageUseCase) executeWithTransaction(ctx context.Context, req *productvariantimagepb.DeleteProductVariantImageRequest) (*productvariantimagepb.DeleteProductVariantImageResponse, error) {
	...
	if err != nil {
		translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "product_variant_image.errors.transaction_failed", "Transaction execution failed [DEFAULT]")
...
func (uc *DeleteProductVariantImageUseCase) executeCore(ctx context.Context, req *productvariantimagepb.DeleteProductVariantImageRequest) (*productvariantimagepb.DeleteProductVariantImageResponse, error) {
	...
	permission := entityid.EntityPermission(entityid.ProductVariantImage, entityid.ActionDelete)
	...
	if err != nil {
		translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "product_variant_image.errors.deletion_failed", "Product variant image deletion failed [DEFAULT]")
		return nil, fmt.Errorf("%s: %w", translatedError, err)
	}
	return resp, nil
}
...
func (uc *DeleteProductVariantImageUseCase) validateInput(ctx context.Context, req *productvariantimagepb.DeleteProductVariantImageRequest) error {
	...
	if req.Data.Id == "" {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "product_variant_image.validation.id_required", "Product variant image ID is required [DEFAULT]"))
	}
	return nil
}
