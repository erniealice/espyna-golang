...
func (uc *UpdateResourceUseCase) validateInput(ctx context.Context, req *resourcepb.UpdateResourceRequest) error {
	...
	if req.Data.ProductId == "" {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "resource.validation.product_id_required", "Product ID is required for the resource [DEFAULT]"))
	}
	return nil
}
