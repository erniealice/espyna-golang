...
func (uc *UpdatePlanSettingsUseCase) Execute(ctx context.Context, req *plansettingspb.UpdatePlanSettingsRequest) (*plansettingspb.UpdatePlanSettingsResponse, error) {
	...
	if err := authcheck.Check(ctx, uc.services.Authorizer, uc.services.Translator,
		entityid.PlanSettings, entityid.ActionUpdate); err != nil {
		return nil, err
	}
	...
	if err != nil {
		return nil, fmt.Errorf(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "plan_settings.errors.update_failed", "plan settings update failed: %s"), err.Error())
	}
...
func (uc *UpdatePlanSettingsUseCase) validateInput(ctx context.Context, req *plansettingspb.UpdatePlanSettingsRequest) error {
	if req == nil {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "plan_settings.validation.request_required", "request is required"))
	}
	if req.Data == nil {
	...
	if req.Data.Id == "" {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "plan_settings.validation.id_required", "plan settings ID is required"))
	}
	return nil
}
...
func (uc *UpdatePlanSettingsUseCase) validateBusinessRules(ctx context.Context, planSettings *plansettingspb.PlanSettings) error {
	...
	if len(planSettings.Id) < 3 {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "plan_settings.validation.id_too_short", "plan settings ID must be at least 3 characters long"))
	}
