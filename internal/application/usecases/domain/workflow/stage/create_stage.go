...
func (uc *CreateStageUseCase) Execute(ctx context.Context, req *stagepb.CreateStageRequest) (*stagepb.CreateStageResponse, error) {
	...
	if err := authcheck.Check(ctx, uc.services.Authorizer, uc.services.Translator,
		"stage", entityid.ActionCreate); err != nil {
		return nil, err
	}
...
func (uc *CreateStageUseCase) executeWithTransaction(ctx context.Context, enrichedStage *stagepb.Stage) (*stagepb.CreateStageResponse, error) {
	...
	err := uc.services.Transactor.ExecuteInTransaction(ctx, func(txCtx context.Context) error {
		...
		if err != nil {
			translatedError := contextutil.GetTranslatedMessageWithContext(txCtx, uc.services.Translator, "stage.errors.creation_failed", "Stage creation failed [DEFAULT]")
...
func (uc *CreateStageUseCase) applyBusinessLogic(stage *stagepb.Stage) *stagepb.Stage {
	...
	if stage.Id == "" {
		if uc.services.IDGenerator != nil {
			stage.Id = uc.services.IDGenerator.GenerateID()
		} else {
...
func (uc *CreateStageUseCase) validateForeignKeys(ctx context.Context, stage *stagepb.Stage) error {
	...
	if stage.WorkflowInstanceId == "" {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "stage.validation.workflow_instance_id_required", "Workflow instance ID is required for stages [DEFAULT]"))
	}
	...
	if err != nil {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "stage.errors.workflow_instance_not_found", "Workflow instance not found [DEFAULT]"))
	}
	if workflowRes == nil || len(workflowRes.Data) == 0 {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "stage.errors.workflow_instance_not_found", "Workflow instance not found [DEFAULT]"))
	}
	if !workflowRes.Data[0].Active {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "stage.errors.workflow_instance_inactive", "Workflow instance is inactive [DEFAULT]"))
	}

	// Foreign key validation: Stage template must exist and be active
	if stage.StageTemplateId == "" {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "stage.validation.stage_template_id_required", "Stage template ID is required for stages [DEFAULT]"))
	}
	...
	if err != nil {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "stage.errors.stage_template_not_found", "Stage template not found [DEFAULT]"))
	}
	if stageTemplateRes == nil || len(stageTemplateRes.Data) == 0 {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "stage.errors.stage_template_not_found", "Stage template not found [DEFAULT]"))
	}
	if !stageTemplateRes.Data[0].Active {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "stage.errors.stage_template_inactive", "Stage template is inactive [DEFAULT]"))
	}
...
func (uc *CreateStageUseCase) validateBusinessRules(ctx context.Context, stage *stagepb.Stage) error {
	...
	if stage.Name == "" {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "stage.validation.name_required", "Stage name is required [DEFAULT]"))
	}
	...
	if stage.DateDue != nil && *stage.DateDue > 0 {
		if *stage.DateDue < time.Now().UnixMilli() {
			return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.Translator, "stage.validation.due_date_past", "Due date cannot be in the past [DEFAULT]"))
		}
	}
