...
// Package client provides comprehensive tests for the client deletion use case.
//
...
//   - Run specific tests: go test -tags="mock_db,mock_auth" -run TestDeleteClientUseCase_Execute_Success
//   - Set environment variables: TEST_USER_ID="admin" TEST_BUSINESS_TYPE="fitness_center" go test
//
// Test Codes:
//   - ESPYNA-TEST-ENTITY-CLIENT-DELETE-SUCCESS-v1.0: Success
//   - ESPYNA-TEST-ENTITY-CLIENT-DELETE-NOT-FOUND-v1.0: NotFound
//   - ESPYNA-TEST-ENTITY-CLIENT-DELETE-EMPTY-ID-v1.0: EmptyId
//
// Data Sources:
...
func TestDeleteClientUseCase_Execute_Success(t *testing.T) {
	testCode := "ESPYNA-TEST-ENTITY-CLIENT-DELETE-SUCCESS-v1.0"
	testutil.SetTestCode(t, testCode)
	testutil.LogTestExecution(t, testCode, "Success", true)

	ctx := testutil.CreateTestContext()
	businessType := testutil.GetTestBusinessType()
	useCase := createTestDeleteClientUseCase(businessType, false)

	// This ID will be "deleted" from the mock repository
	existingID := "student-003"

	req := &clientpb.DeleteClientRequest{
	...
	// Verify the client is actually deleted - use same repository instance
...
func TestDeleteClientUseCase_Execute_NotFound(t *testing.T) {
	testCode := "ESPYNA-TEST-ENTITY-CLIENT-DELETE-NOT-FOUND-v1.0"
	testutil.SetTestCode(t, testCode)
...
func TestDeleteClientUseCase_Execute_EmptyId(t *testing.T) {
	testCode := "ESPYNA-TEST-ENTITY-CLIENT-DELETE-EMPTY-ID-v1.0"
	testutil.SetTestCode(t, testCode)
