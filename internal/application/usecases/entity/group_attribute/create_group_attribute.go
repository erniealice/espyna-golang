...
func (uc *CreateGroupAttributeUseCase) Execute(ctx context.Context, req *groupattributepb.CreateGroupAttributeRequest) (*groupattributepb.CreateGroupAttributeResponse, error) {
	...
	if uc.services.AuthorizationService != nil && uc.services.AuthorizationService.IsEnabled() {
		...
		if userID == "" {
			return nil, errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.TranslationService, "group_attribute.errors.user_not_authenticated", "User not authenticated [DEFAULT]"))
		}

		// Note: For group attributes, we need to get workspace from group
		group, err := uc.repositories.Group.ReadGroup(ctx, &grouppb.ReadGroupRequest{
			Data: &grouppb.Group{Id: req.Data.GroupId},
		})
		if err != nil {
			translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.TranslationService, "group_attribute.errors.group_fetch_failed_auth", "Failed to get group for authorization [DEFAULT]")
			return nil, fmt.Errorf("%s: %w", translatedError, err)
		}
		if group == nil || group.Data == nil || len(group.Data) == 0 {
			return nil, errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.TranslationService, "group_attribute.errors.group_not_found", ""))
		}

		// TODO: Re-enable workspace-scoped authorization check once Group.WorkspaceId is available
		// workspaceID := group.Data[0].WorkspaceId
		// if workspaceID == "" {
		// 	return nil, fmt.Errorf("workspace ID is required for authorization check")
		// }

		// permission := ports.EntityPermission(ports.EntityGroupAttribute, ports.ActionCreate)
...
func (uc *CreateGroupAttributeUseCase) validateInput(ctx context.Context, req *groupattributepb.CreateGroupAttributeRequest) error {
	...
	if req.Data.GroupId == "" {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.TranslationService, "group_attribute.validation.group_id_required", ""))
	}
	if req.Data.AttributeId == "" {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.TranslationService, "group_attribute.validation.attribute_id_required", ""))
	}
	if req.Data.Value == "" {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.TranslationService, "group_attribute.validation.value_required", ""))
	}
	return nil
}
...
func (uc *CreateGroupAttributeUseCase) validateEntityReferences(ctx context.Context, groupAttribute *groupattributepb.GroupAttribute) error {
	...
	if groupAttribute.GroupId != "" {
		...
		if group == nil || group.Data == nil || len(group.Data) == 0 {
			translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.TranslationService, "group_attribute.errors.group_not_found", "")
	...
	if groupAttribute.AttributeId != "" {
		...
		if attribute == nil || attribute.Data == nil || len(attribute.Data) == 0 {
			translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.TranslationService, "group_attribute.errors.attribute_not_found", "")
