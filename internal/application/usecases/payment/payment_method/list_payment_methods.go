...
func (uc *ListPaymentMethodsUseCase) Execute(ctx context.Context, req *paymentmethodpb.ListPaymentMethodsRequest) (*paymentmethodpb.ListPaymentMethodsResponse, error) {
	...
	if uc.services.AuthorizationService != nil {
		if enabled, ok := uc.services.AuthorizationService.(interface{ IsEnabled() bool }); ok && enabled.IsEnabled() {
			...
			if authorized, err := uc.services.AuthorizationService.HasPermission(ctx, uid, ports.EntityPermission(ports.EntityPaymentMethod, ports.ActionList)); err != nil || !authorized {
