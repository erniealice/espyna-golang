...
func (uc *UpdatePaymentProfileUseCase) Execute(ctx context.Context, req *paymentprofilepb.UpdatePaymentProfileRequest) (*paymentprofilepb.UpdatePaymentProfileResponse, error) {
	...
	if uc.services.AuthorizationService != nil {
		if enabled, ok := uc.services.AuthorizationService.(interface{ IsEnabled() bool }); ok && enabled.IsEnabled() {
			...
			if authorized, err := uc.services.AuthorizationService.HasPermission(ctx, uid, ports.EntityPermission(ports.EntityPaymentProfile, ports.ActionUpdate)); err != nil || !authorized {
