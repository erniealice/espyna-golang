// Package registrar is the central blank-import host for
...
//
// This package lives at `service/registrar/` — a sibling sub-package
// (separate Go package from `service`, so no cycle):
//
//   - `service/registrar` imports child packages (transitive: `service`).
//   - children import `service` to register.
//   - `service` imports nothing from `service/registrar` (no cycle).
//
...
// `initializers/service/<X>.go` (no per-candidate construction call).
//
