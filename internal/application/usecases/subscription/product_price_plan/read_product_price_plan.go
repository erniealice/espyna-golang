...
func (uc *ReadProductPricePlanUseCase) Execute(ctx context.Context, req *productpriceplanpb.ReadProductPricePlanRequest) (*productpriceplanpb.ReadProductPricePlanResponse, error) {
	...
	if err != nil {
		...
		translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.TranslationService, "product_price_plan.errors.read_failed", "Failed to read product price plan")
		return nil, fmt.Errorf("%s: %w", translatedError, err)
	}

	if resp == nil || resp.Data == nil || len(resp.Data) == 0 {
		translatedError := contextutil.GetTranslatedMessageWithContext(ctx, uc.services.TranslationService, "product_price_plan.errors.not_found", "Product price plan with ID \"{id}\" not found [DEFAULT]")
