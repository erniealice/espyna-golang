...
func (uc *CreateSecurityDepositUseCase) Execute(ctx context.Context, req *securitydepositpb.CreateSecurityDepositRequest) (*securitydepositpb.CreateSecurityDepositResponse, error) {
	...
	if uc.services.TransactionService != nil && uc.services.TransactionService.SupportsTransactions() {
		...
		err := uc.services.TransactionService.ExecuteInTransaction(ctx, func(txCtx context.Context) error {
			...
			if err != nil {
				translatedError := contextutil.GetTranslatedMessageWithContext(txCtx, uc.services.TranslationService, "security_deposit.errors.creation_failed", "Security deposit creation failed [DEFAULT]")
...
func (uc *CreateSecurityDepositUseCase) validateInput(ctx context.Context, req *securitydepositpb.CreateSecurityDepositRequest) error {
	...
	if req.Data == nil {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.TranslationService, "security_deposit.validation.data_required", "[ERR-DEFAULT] Security deposit data is required"))
	}

	req.Data.CounterpartyName = strings.TrimSpace(req.Data.CounterpartyName)
	if req.Data.CounterpartyName == "" {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.TranslationService, "security_deposit.validation.counterparty_required", "[ERR-DEFAULT] Counterparty name is required"))
	}
	if req.Data.Amount <= 0 {
		return errors.New(contextutil.GetTranslatedMessageWithContext(ctx, uc.services.TranslationService, "security_deposit.validation.amount_required", "[ERR-DEFAULT] Amount must be greater than zero"))
	}
	return nil
}
...
func (uc *CreateSecurityDepositUseCase) enrichSecurityDepositData(sd *securitydepositpb.SecurityDeposit) error {
	now := time.Now()
	if sd.Id == "" {
		sd.Id = uc.services.IDService.GenerateID()
	}
