...
type GetWorkflowListPageDataRepositories struct {
...
type GetWorkflowListPageDataUseCase struct {
	repositories GetWorkflowListPageDataRepositories
	services     GetWorkflowListPageDataServices
	processor    *listdata.ListDataProcessor
}

// NewGetWorkflowListPageDataUseCase creates a new GetWorkflowListPageDataUseCase
func NewGetWorkflowListPageDataUseCase(
	repositories GetWorkflowListPageDataRepositories,
	services GetWorkflowListPageDataServices,
) *GetWorkflowListPageDataUseCase {
	return &GetWorkflowListPageDataUseCase{
...
// Execute performs the get workflow list page data operation
func (uc *GetWorkflowListPageDataUseCase) Execute(
	ctx context.Context,
...
// executeWithTransaction executes workflow list page data retrieval within a transaction
func (uc *GetWorkflowListPageDataUseCase) executeWithTransaction(
	ctx context.Context,
	req *workflowpb.GetWorkflowListPageDataRequest,
) (*workflowpb.GetWorkflowListPageDataResponse, error) {
	var result *workflowpb.GetWorkflowListPageDataResponse
	...
	err := uc.services.TransactionService.ExecuteInTransaction(ctx, func(txCtx context.Context) error {
		...
		if err != nil {
			return fmt.Errorf(contextutil.GetTranslatedMessageWithContext(
				...
				"workflow.errors.list_page_data_failed",
				"workflow list page data retrieval failed: %w",
			), err)
		}
		result = res
		return nil
...
// executeCore contains the core business logic for getting workflow list page data
func (uc *GetWorkflowListPageDataUseCase) executeCore(
	ctx context.Context,
	req *workflowpb.GetWorkflowListPageDataRequest,
) (*workflowpb.GetWorkflowListPageDataResponse, error) {
	// First, get all workflows from the repository
	listReq := &workflowpb.ListWorkflowsRequest{}
	listResp, err := uc.repositories.Workflow.ListWorkflows(ctx, listReq)
	if err != nil {
		return nil, fmt.Errorf(contextutil.GetTranslatedMessageWithContext(
			ctx,
			uc.services.TranslationService,
			"workflow.errors.list_failed",
			"failed to retrieve workflows: %w",
		), err)
	}
	...
	if listResp == nil || len(listResp.Data) == 0 {
		...
		return &workflowpb.GetWorkflowListPageDataResponse{
			WorkflowList:  []*workflowpb.Workflow{},
	...
	if err != nil {
		return nil, fmt.Errorf(contextutil.GetTranslatedMessageWithContext(
			ctx,
			uc.services.TranslationService,
			"workflow.errors.processing_failed",
			"failed to process workflow list data: %w",
		), err)
	}

	// Convert processed items back to workflow protobuf format
	workflows := make([]*workflowpb.Workflow, len(result.Items))
	for i, item := range result.Items {
		if workflow, ok := item.(*workflowpb.Workflow); ok {
			...
			return nil, errors.New(contextutil.GetTranslatedMessageWithContext(
				ctx,
				uc.services.TranslationService,
				"workflow.errors.type_conversion_failed",
				"failed to convert item to workflow type",
			))
		}
	}
...
func (uc *GetWorkflowListPageDataUseCase) validateInput(
	ctx context.Context,
	req *workflowpb.GetWorkflowListPageDataRequest,
) error {
	if req == nil {
...
func (uc *GetWorkflowListPageDataUseCase) validatePagination(
	ctx context.Context,
	...
	if pagination.Limit < 0 || pagination.Limit > 100 {
		return errors.New(contextutil.GetTranslatedMessageWithContext(
			ctx,
			uc.services.TranslationService,
			"workflow.validation.invalid_limit",
	...
	case *commonpb.PaginationRequest_Offset:
		if method.Offset.Page < 1 {
			return errors.New(contextutil.GetTranslatedMessageWithContext(
				ctx,
				uc.services.TranslationService,
				"workflow.validation.invalid_page",
	...
	case *commonpb.PaginationRequest_Cursor:
		...
		if method.Cursor.Token == "" {
			return errors.New(contextutil.GetTranslatedMessageWithContext(
				ctx,
				uc.services.TranslationService,
				"workflow.validation.invalid_cursor",
...
func (uc *GetWorkflowListPageDataUseCase) validateFilters(
	ctx context.Context,
	...
	if len(filters.Filters) == 0 {
		return errors.New(contextutil.GetTranslatedMessageWithContext(
			ctx,
			uc.services.TranslationService,
			"workflow.validation.empty_filters",
	...
	for i, filter := range filters.Filters {
		if filter.Field == "" {
			return fmt.Errorf(contextutil.GetTranslatedMessageWithContext(
				ctx,
				uc.services.TranslationService,
				"workflow.validation.filter_field_required",
				"filter field is required for filter %d",
			), i)
		}

		// Validate that the field exists on the workflow entity
		if !uc.isValidWorkflowField(filter.Field) {
			return fmt.Errorf(contextutil.GetTranslatedMessageWithContext(
				ctx,
				uc.services.TranslationService,
				"workflow.validation.invalid_filter_field",
...
func (uc *GetWorkflowListPageDataUseCase) validateSort(
	ctx context.Context,
	...
	if len(sort.Fields) == 0 {
		return errors.New(contextutil.GetTranslatedMessageWithContext(
			ctx,
			uc.services.TranslationService,
			"workflow.validation.empty_sort_fields",
	...
	for i, sortField := range sort.Fields {
		if sortField.Field == "" {
			return fmt.Errorf(contextutil.GetTranslatedMessageWithContext(
				ctx,
				uc.services.TranslationService,
				"workflow.validation.sort_field_required",
				"sort field is required for sort field %d",
			), i)
		}

		// Validate that the field exists on the workflow entity
		if !uc.isValidWorkflowField(sortField.Field) {
			return fmt.Errorf(contextutil.GetTranslatedMessageWithContext(
				ctx,
				uc.services.TranslationService,
				"workflow.validation.invalid_sort_field",
...
func (uc *GetWorkflowListPageDataUseCase) validateSearch(
	ctx context.Context,
	...
	if search.Query == "" {
		return errors.New(contextutil.GetTranslatedMessageWithContext(
			ctx,
			uc.services.TranslationService,
			"workflow.validation.empty_search_query",
	...
	if search.Options != nil {
		...
		for _, field := range search.Options.SearchFields {
			if !uc.isValidWorkflowField(field) {
				return fmt.Errorf(contextutil.GetTranslatedMessageWithContext(
					ctx,
					uc.services.TranslationService,
					"workflow.validation.invalid_search_field",
		...
		if search.Options.MaxResults < 0 || search.Options.MaxResults > 1000 {
			return errors.New(contextutil.GetTranslatedMessageWithContext(
				ctx,
				uc.services.TranslationService,
				"workflow.validation.invalid_max_results",
...
// isValidWorkflowField checks if a field name is valid for workflow filtering/sorting/searching
func (uc *GetWorkflowListPageDataUseCase) isValidWorkflowField(field string) bool {
