...
// AuthenticationMiddleware provides authentication middleware for Fiber v3
...
// RequireAuth is a Fiber v3 middleware that validates authentication tokens
func (m *AuthenticationMiddleware) RequireAuth() fiber.Handler {
	return func(c fiber.Ctx) error {
		...
		resp, err := m.authService.VerifyToken(ctx, req)
		if err != nil {
...
// GetUserFromFiberContext extracts user information from Fiber v3 context
...
// GetIdentityFromFiberContext extracts the full identity from Fiber v3 context
...
// GetAuthTokenFromFiberContext extracts the JWT token from Fiber v3 context
// This replaces the old AuthToken struct with the proto-based JwtToken
func GetAuthTokenFromFiberContext(c fiber.Ctx) (*authpb.JwtToken, bool) {
	tokenVal := c.Locals("jwt_token")
	if tokenVal == nil {
		return nil, false
	}

	token, ok := tokenVal.(*authpb.JwtToken)
	return token, ok
}