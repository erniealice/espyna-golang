...
func (s *Server) setupRoutes() {
	fmt.Printf("🔍 Setting up Fiber v3 routes...\n")
	...
	if routeManager != nil {
		...
		// Install each route on the Fiber v3 app
	...
	fmt.Printf("✅ Fiber v3 route setup completed\n")
}

// installRoute installs a single route on the Fiber v3 app
func (s *Server) installRoute(route *routing.Route) {
	// Create handler function
	handler := s.createRouteHandler(route)

	// Register route with Fiber v3 app
	switch route.Method {
	case "GET":
...
// createRouteHandler creates a Fiber v3 handler function from a routing.Route
func (s *Server) createRouteHandler(route *routing.Route) fiber.Handler {
	return func(c fiber.Ctx) error {
		// Enhanced logging: Request start
		fmt.Printf("🚀 [FIBER V3 ROUTE HANDLER] Incoming request: %s %s\n", c.Method(), c.Path())
...
// extractPathParams extracts path parameters from the Fiber v3 request
func (s *Server) extractPathParams(c fiber.Ctx) map[string]string {
	// Note: Fiber v3 doesn't have a direct way to get all params as a map
...
// extractQueryParams extracts query parameters from the Fiber v3 request
func (s *Server) extractQueryParams(c fiber.Ctx) map[string]string {
	// Fiber v3 has a simplified Queries() method that returns all query params
	// as a map of strings to strings (first value of each key)
	return c.Queries()
}

// extractHeaders extracts headers from the Fiber v3 request
func (s *Server) extractHeaders(c fiber.Ctx) map[string]string {
	headers := make(map[string]string)
	// Fiber v3 has improved header iteration
