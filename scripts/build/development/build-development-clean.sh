...
#   -a, --all-business-types    Include mock data for all business types (default: true)
#   -h, --help                  Show this help message
#
# EXAMPLES:
#   ./build-development-clean.sh
#       Ultra-minimal development server with all mock providers
#
#   ./build-development-clean.sh --verbose --race
...
show_help() {
    ...
    echo "  -a, --all-business-types    Include mock data for all business types (default: true)"
...
SECONDARY_TAGS_ARRAY=("${BUILD_TAGS[@]:1}")  # Remove "vanilla" from the array
SECONDARY_TAGS_STRING=$(IFS=','; echo "${SECONDARY_TAGS_ARRAY[*]}")

echo -e "${MAGENTA}Executing development build...${NC}"
echo -e "${GRAY}Command: ./scripts/build-with-tags.sh --framework vanilla --secondary-tags '$SECONDARY_TAGS_STRING' --output $OUTPUT${NC}"
echo ""
...
if ./scripts/build-with-tags.sh "${BUILD_ARGS[@]}"; then
    ...
    if [[ -f "$OUTPUT_PATH" ]]; then
        ...
        if command -v bc >/dev/null 2>&1; then
            ...
            echo -e "${MAGENTA}Binary size: ${BINARY_SIZE_DISPLAY} MB (development-optimized)${NC}"
            
            # Size comparison with enterprise build
            COMPARISON_SIZE=70  # Typical enterprise build size
            SIZE_REDUCTION=$(echo "scale=1; ($COMPARISON_SIZE - $BINARY_SIZE_DISPLAY) / $COMPARISON_SIZE * 100" | bc)
            ...
            if (( $(echo "$BINARY_SIZE_DISPLAY < $MINIMAL_SIZE" | bc -l) )); then
                DEV_ADVANTAGE=$(echo "scale=1; ($MINIMAL_SIZE - $BINARY_SIZE_DISPLAY) / $MINIMAL_SIZE * 100" | bc)
            ...
            # Simple comparison with enterprise build
            if [[ $BINARY_SIZE_MB -lt 35 ]]; then
                echo -e "${GREEN}Size reduction: Significantly smaller than enterprise builds (typically 70MB)${NC}"
                
                # Compare with minimal API build
                if [[ $BINARY_SIZE_MB -lt 15 ]]; then
                    echo -e "${CYAN}Even smaller: Comparable to minimal API builds${NC}"
                fi
            fi
        fi
    fi
